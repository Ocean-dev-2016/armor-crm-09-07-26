			...
			if (isset($_REQUEST['username']) && isset($_REQUEST['password'])  && isset($_REQUEST['refreshToken']))
				{
					...
					$inser_value = array(
					$data['id'],$imei,$refreshToken,$AppVersionName,$AppName,$BatteryPercent,$Device,$Hardware,$Manufacturer,$Model,$OsVersion,$SdkVersion,$AvailableInternalMemorySize,$TotalInternalMemorySize,$NetworkType,$Operator,$PhoneNumber,$sIMSerial,$isWifiEnabled,$isNetworkAvailable,$isGps
					);
			...
			if(isset($_REQUEST['sales_executive_id']) && isset($_REQUEST['longitude']) && isset($_REQUEST['latitude']))
				...
				// 		$data=$db->rp_getData("salesexecutive_tracking","*","id=".$e_id ."");
							...
							if($e_id!=0)
							{
								...
								"developer_msg"=>"You got it!!",
								"result"=>"Tracking Detail Add Successfully!!"
			...
			else if($service=='get_inquiry_report' || $service==46)
		   {
			...
			else if($service=='get_expense' || $service==22)
			{
				...
				if($sales_executive_id	=(isset($_REQUEST['sales_executive_id'])&& $_REQUEST['sales_executive_id']!="")?$_REQUEST['sales_executive_id']:"")
				{
					...
					{
						// When dates are not provided, show full expense history (no date restriction).
					}
		...
		else if($service=='add_sales_executive_tracking' || $service==32)
		{
	        ...
	        if(isset($_REQUEST['sales_id']) && $body!="")
			{
				...
				if($ins)
				{
					...
					//if($dt)
					//{
						//while($row=mysqlii_fetch_assoc($dt))
						//{
				...
				//	}
					$ack=array( "ack"=>1,
						"ack_msg"=>"Sales Executive Tracking Add Successfully!!",
						"developer_msg"=>"You got it!!",
						"result"=>"Sales Executive Tracking Add Successfully!!"
				// 		"result"=>$r
						);
		...
		else if($service=='get_reject_expense' || $service==34)
		{
			...
			if($sales_executive_id	=(isset($_REQUEST['sales_executive_id'])&& $_REQUEST['sales_executive_id']!="")?$_REQUEST['sales_executive_id']:"")
			{
				...
				if(!empty($result))
				{
					...
					$ack=array( "ack"=>1,"ack_msg"=>"Rejected Expense History Found!!",
		...
		else if($service=='get_active_app' || $service==48)
		{
			...
			if($application_info_r>0)
			{
				...
				$ack=array( "ack"=>1,
						...
						"developer_msg"=>"You got it!!",
						"result"=>$result,"download_path"=>DOWNLOAD_PATH,"catalog_title"=>CATALOG_TITLE,"visiting_card_download_path"=>VISITING_DOWNLOAD_PATH,"visiting_card_title"=>VISITING_TITLE
						);
						$db->printJSON($ack);
			}
			else	
			{
				$ack=array( "ack"=>0,
						...
						"developer_msg"=>"Sorry We can't Procced!","download_path"=>DOWNLOAD_PATH,"catalog_title"=>CATALOG_TITLE,"visiting_card_download_path"=>VISITING_DOWNLOAD_PATH,"visiting_card_title"=>VISITING_TITLE
						);
						$db->printJSON($ack);
			}					
...
//----------------------------------------------------------------------------//
		...
		else if($service=='get_dealer' || $service==78)
		{
        	...
        	$detail['customer_flag']=isset($_REQUEST['customer_flag'])?$_REQUEST['customer_flag']:"0";
        	$detail['type_of_company']=isset($_REQUEST['company_id'])?$_REQUEST['company_id']:"0";
        	$detail['is_class_area_filter']=isset($_REQUEST['is_class_area_filter'])?$_REQUEST['is_class_area_filter']:"0";
            $executive=new Executive();
            if($detail['cid']!=""){
            	//echo "hello234";exit();
