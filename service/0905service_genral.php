...
//#login For Sales Officer---------------------------------// 
		...
		else if($service=="today_work_count" || $service==212)
		{
			...
			if ($executive_id!="") 
			{
				...
				if($insert)
				{
					$ack=array("ack"=>1,"ack_msg"=>"Customer Plan Add Successfully","developer_msg"=>"Customer Plan Add  Successfully!!");		
				}
				else
				{
					$ack=array("ack"=>0,"ack_msg"=>"Customer Plan Add Failed","developer_msg"=>"Customer Plan Add Failed!!");
				}
			...
			if ($executive_id != "") {
				...
				if ($isUpdated) {
					$ack = array(
						...
						"developer_msg" => "Customer plan updated successfully"
					);
				} else {
					$ack = array(
						"ack"     => 0,
			...
			if ($sales_executive_id != "" && $month != "" && $year != "") {
				...
				if ($sales_vs_plan_r) {
					while ($row = mysqli_fetch_assoc($sales_vs_plan_r)) {
						...
						$excutive_name_data = $db->rp_getData("executive", "cname,company_name", "isDelete=0 AND id='" . $executive_id . "'");
