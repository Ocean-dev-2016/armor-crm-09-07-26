		...
		<div class="col-md-12 col-sm-12 co-xs-12 col-lg-12">
			<div class="portlet light ">
				<div class="portlet-title">
					<div class="caption ">
						<span class="caption-subject bold uppercase font-dark">Class Area Allocation Pending Report</span>
					</div>
				</div>
						...
						<table class="table table-hover table-light">
							...
							<tbody>
								<?php
									...
									$executive_id = implode(",",$executive_id);

									$customer_list = $db->rp_getData("executive","id,cname,company_name,type_of_executive,state,city","id NOT IN (".$executive_id.") AND isDelete=0","id DESC",0);
									while($customer_list_r = mysqli_fetch_array($customer_list))
									{
										$se_count++;
										?>
										<tr>
											<td><?= $se_count ?></td>
											<td colspan="2"><?= $customer_list_r['company_name'] ?></td>
											<td colspan="2"><?= $customer_list_r['state'] ?></td>
											<td colspan="2"><?= $customer_list_r['city'] ?></td>
											<td colspan="2"><?= $type_of_executive_array[$customer_list_r['type_of_executive']] ?></td>
											<td colspan="2">Class Area Allocation Pending</td>
										</tr>
										<?php
