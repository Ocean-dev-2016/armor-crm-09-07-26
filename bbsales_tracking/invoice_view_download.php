...
<table>
	<tbody class="<?= $cl; ?>">
		...
		<tr>
			<td colspan="16" class=" no-border-top height-5 text-center"><strong>GST - <?= COMPANY_GST ?></strong></td>
		</tr>

		<tr>
			<td colspan="16" class="no-border-bottom  height-5 text-center"><strong>PAN - <?= COMPANY_PAN ?></strong></td>
		</tr>
		...
		<tr class="vertical-top">
			<td colspan="6" class="no-border-bottom no-border-top border-r-width">
				...
				<strong><span><b>GST No :</b> <?=$Distri_data_a['gst']?></span></strong><br/>
			...
			<td colspan="6" class="no-border-bottom no-border-top ">
				...
				<table class="no-border-top no-border-bottom no-border-right no-border-left" style="width: 45mm!important;margin:0px;">
					...
					<tr>
						<td class="no-border-right no-border-bottom no-border-left"><b>P.O. No :</b> <?= $OrderD['po_no']?></td>
					</tr>
					<tr>
						<td class="no-border-right no-border-bottom no-border-left"><b>P.O. Date :</b> 
						<?php

						$po_date=date('d-m-Y', strtotime($OrderD['po_date']));
		...
		if($items1)
		{
			...
			foreach($ITEMS as $item)
			{
				...
				<tr>
					...
					<td colspan="2" class="" style="position: relative;"><?php if($item['weight_id']!=-1){echo strtoupper($product_code); } else { echo strtoupper($product_code); } ?></strong></td>
		...
		<tr class="font-size">
			...
			<td colspan="4" class="" rowspan="11" style="vertical-align: top;">	
				...
				?>
				<span class="font-13"><b>GST Amount</b> : <?php echo ucwords(strtolower($finalgst)); ?></span>
				<br><span class="font-13" ><b>Bill Amount (Words)</b> : <?php echo ucwords(strtolower($grand_total_words1)); ?></span>
			...
			<td colspan="4" rowspan="11" style="vertical-align: top;">
				...
				<table style="width: 45mm!important; vertical-align: top;">
					<tr>
						<td ><b>Total Parcle</b></td>
						<td><?=$get_main_cartoon_count?></td>
					</tr>
					<tr>
						<td ><b>Total Weight</b></td>
						<td ><?=round($get_sum_for_weight,2);?></td>
					</tr>
				</table>
			</td>
		...
		<tr>
			...
			<td colspan="5" class="text-right font-13"><strong><?php echo $currency.' '.$db->rp_number_format(round($total_tax_amt,2)); ?></strong></td>
		</tr>
				...
				else
				{
					?>
						<tr>
							...
							<td colspan="5" class="text-right "><strong><?= (CURR. $db->rp_number_format($finalgst))?></strong></td>
						</tr>
						<tr>
							<td colspan="3" class="text-left"></td>			
							<td colspan="5" class="text-right "></td>
		...
		<tr style="font-size: 16px; background-color: #669B49;" >
			<td colspan="3">
			...
			<td colspan="5" class="text-right"><strong>
				<?php if($cart_detail_d['tcs_amount'] !="" && $cart_detail_d['tcs_amount'] != "0")
				{
					...
					echo CURR.' '.$db->rp_number_format(round($total_tax_amt+$finalgst+$tcs_amount,0));
				}
				else
				{
					echo CURR.' '.$db->rp_number_format(round($total_tax_amt+$finalgst,0));
				}
				?>
				</strong>
			</td>
