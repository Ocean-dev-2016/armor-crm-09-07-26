...
<head>
	...
	<style type="text/css">
		...
		#jqxScrollAreaDownverticalScrollBarjqxgrid,
		#jqxScrollAreaUpverticalScrollBarjqxgrid,
		#jqxScrollBtnUpverticalScrollBarjqxgrid,
		#jqxScrollThumbverticalScrollBarjqxgrid,
...
<body class="page-md">
									...
									<div class="col-md-12" style="margin: 0px  0!important">
										<div class="row">
											...
											<div class="col-md-3 col-xs-3 col-sm-3">
												...
												if ($_REQUEST['type'] == "-1") {
												?>
													<a href="#convertinquiry" style="padding: 7px" class="btn btn-warning " target="#convertinquiry" data-toggle="modal" title="Edit"><i class="fa fa-plus"></i>Assign Inquiry</a>
												<?php
												}
												?>
												<button class="btn btn-danger" id="delete_records" type="button" name="delete"><b><i class="fa fa-trash"></i> Delete Inquiries</b></button>
											</div>
											<div class="col-md-3 col-xs-3 col-sm-3 text-right">
												<div class="form-check">
													<input type="checkbox" class="form-check-input" id="status_check" name="status_check" value="0" <?= ($_REQUEST['status'] == 11 || $_REQUEST['status'] == -2) ? "checked" : ""; ?>>
												...
												<div class="form-inline" role="form">
													<form class="form-inline pull-right" role="form" onSubmit="return searchByName();">
														<div class="form-group">
															<input type="text" style="width: 450px!important" placeholder="Search By Person / Company Name / Mobile Number/ Pincode:  " class="form-control input-large" name="searchName" id="searchName" value="" />
														</div>
														<div class="form-group">
	...
	<div class="modal" id="convertinquiry" role="dialog" aria-labelledby="myModalLabel1">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<form role="form" action="" method="post" id="formLocation">
					<div class="modal-header">
						<h4 class="modal-title" id="myModalLabel1">Assign Inquiries</h4>
					...
					<div class="modal-body">
						<fieldset>
							<label class="test">Inquiry Assigned to</label>
							<select class="form-control b-3" id="inquiry_assign_to" name="inquiry_assign_to">
								<option value="">Select Inquiry Assigned to</option>
								<?php
								$sales_id_r = $db->rp_getData("sales_executive", "*", "isDelete=0 AND isActive=1 AND id!='" . $not_intrested_by . "'", 0);
								if ($sales_id_r) {
					...
					<div class="modal-footer">
						...
						<button type="button" id="save_inquiry" class="btn btn-success">Save </button>
					</div>
				</form>
			</div>
		</div>
	...
	<script type="text/javascript">
		...
		function callAjax(sales_id, date, numRecords, page = "") {
		...
		function GenerateInquiry(id) {
			...
			$.ajax({
				...
				success: function(data) {
					// $("#loading-modal").modal('hide');  
					$('.preloader').fadeOut('slow');
					//	window.open("no_order_inquiry_crud.php?type=0","_blank");
					window.open("no_order_inquiry_crud.php?m=change_to_inquiry&mode=edit&type=0&id=" + id, "_blank");
					//$("#jqxgrid").jqxGrid('updatebounddata');
				}
			});
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (data['ack'] == 1) {
						// $("#loading-modal").modal('hide');  
						$('.preloader').fadeOut('slow');
						window.open("no_order_inquiry_manage.php?type=1", "_blank");
						$("#jqxgrid").jqxGrid('updatebounddata');
					} else if (data['ack'] == 2) {
	...
	<script type="text/javascript">
		$('#convertinquiry').on('click', function(e) {
			var inquiry_id = [];
			$(".inquiry_multi:checked").each(function() {
				inquiry_id.push($(this).data('inquiry_id'));
			});
			if (inquiry_id.length <= 0) {
				toastr.error("please select at least one records.", "Error");

			}
		});

		$('#save_inquiry').on('click', function(e) {
			var inquiry_created_by = "";
			var inquiry_assign_to = $('#inquiry_assign_to').val();
			...
			if (inquiry_id.length <= 0) {
				toastr.error("Please select at least one records.", "Error");
			} else if (inquiry_assign_to == "") {
				toastr.error("Please Assign to", "Error");
			} else {
				WRN_PROFILE_DELETE = "Are you sure you want to Update Selected Row";
				var checked = confirm(WRN_PROFILE_DELETE);
				if (checked == true) {
					var selected_values = inquiry_id.join(",");
					$.ajax({
						type: "POST",
						url: "convert_inquiry_update_ajax.php",
						data: 'id=' + selected_values + '&inquiry_created_by=' + inquiry_created_by + '&inquiry_assign_to=' + inquiry_assign_to,
		...
		$('#delete_records').on('click', function(e) {
			...
			if (inquiry_id.length <= 0) {
				...
				WRN_PROFILE_DELETE = "Are you sure you want to Delete Selected Row";
				var checked = confirm(WRN_PROFILE_DELETE);
				if (checked == true) {
					var selected_values = inquiry_id.join(",");
					$.ajax({
						type: "POST",
						url: "delete_multiple_inquiry_ajax.php",
