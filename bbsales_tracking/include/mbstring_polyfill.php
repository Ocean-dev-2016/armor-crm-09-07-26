<?php
/**
 * Minimal mbstring polyfill for hosts where php_mbstring is disabled.
 * Enough for mPDF 6 basic HTML→PDF (CP Share PDF). PHP 5.6 compatible.
 */
if (!function_exists('mb_strlen')) {
	...
	function _cp_mb_encoding($encoding = null)
	{
	...
	function mb_strlen($str, $encoding = null)
	{
		...
		if (function_exists('iconv_strlen')) {
			$len = @iconv_strlen($str, _cp_mb_encoding($encoding));
			if ($len !== false) {
				return $len;
			}
		}
		/* UTF-8 length via preg */
	...
	function mb_substr($str, $start, $length = null, $encoding = null)
	{
		...
		if (function_exists('iconv_substr')) {
			if ($length === null) {
				$res = @iconv_substr($str, $start, 2147483647, _cp_mb_encoding($encoding));
			} else {
				$res = @iconv_substr($str, $start, $length, _cp_mb_encoding($encoding));
			}
			if ($res !== false) {
				return $res;
			}
	...
	function mb_convert_case($str, $mode, $encoding = null)
	{
		...
		/* Title case — ASCII-oriented fallback */
