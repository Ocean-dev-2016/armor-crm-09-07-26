...
if($ss_id!="")
{
	...
	$result=array();
	$area_r = $db->rp_getData("executive_map_area","area_id","class_id = '".$_POST["class_id"]."' AND executive_id = '".$ss_id."' AND isDelete=0","",0);
	if($area_r)
	{
		while($area_d = mysqli_fetch_assoc($area_r))
			...
			$area=$db->rp_getValue("area","name","id='".$area_d['area_id']."' AND isDelete=0");
				if(!$json)
				{?>
				<option value="<?php echo $area_d['area_id']; ?>" <?php echo (in_array($area_d['area_id'],$user_info))?"selected":""; ?>><?php echo $area; ?></option>
...
else if($dd_id!="")
{
	...
	$area_r =$db->rp_getData("executive_map_area","area_id","class_id = '".$_POST["class_id"]."' AND executive_id = '".$dd_id."'","",0);
	if($area_r)
	{
		while($area_d = mysqli_fetch_assoc($area_r))
			{
...
{
		$class_d=$db->rp_getData("sales_executive_map_area","*","class_id='".$class_id."' AND isDelete=0","",0);
		$user_info=array();
		while($class_r=mysqli_fetch_assoc($class_d)){
			$area_name=$db->rp_getValue("area","name","id='".$class_r['area_id']."'");
			$user_info=array("area_id"=>$class_r['area_id']);
		}
		
		$area_r = $db->rp_getData("area","*","class_id = '".$class_id."' AND isDelete=0","",0);
		if($area_r)
			while($area_d = mysqli_fetch_assoc($area_r))
			{
				if(!$json)
