...
$orderViewEmbedded = (basename($_SERVER['SCRIPT_NAME']) === 'order_viewer.php');
$orderViewStandalone = !$orderViewEmbedded;
...
$orderPrintTitle = 'Pro Forma Invoice';
if (!empty($cart_detail_d['order_no'])) {
	$orderPrintTitle .= ' - ' . trim($cart_detail_d['order_no']);
}
if (!empty($cart_detail_d['company_name'])) {
	$orderPrintTitle .= ' - ' . trim($cart_detail_d['company_name']);
}
$orderPrintTitle = preg_replace('/[\\\\\/:*?"<>|]+/', '-', $orderPrintTitle);
$orderPrintTitle = preg_replace('/\s+/', ' ', $orderPrintTitle);
...
<head>
	<title><?= htmlspecialchars($orderPrintTitle, ENT_QUOTES, 'UTF-8') ?></title>
	...
	<table class="quote-table">
		<tbody>
			...
			<td class="quote-header-cell" colspan="16">
				...
				?>
					<img class="quote-header-img" src="../images/craftbox_header.jpg" alt="Header">
				<?php
				}
				?>
			</td>
			...
			<tr style="background-color: <?= VIEW_COLOR ?>; color: #000;">
				<td colspan="16" align="center" style="color: #000;"><b>PRO FORMA INVOICE</b></td>
			</tr>
		</tbody>
	</table>
	<table class="quote-table">
		<tbody class="<?= $cl; ?>">
			<tr>
				...
				<td colspan="8" style="text-align: left;vertical-align: top;">
					...
					<?php if ($cart_detail_d['shipping_address'] != "") { ?>
						<p>
		...
		<table class="quote-table product-items-table">
			<tbody>
				<tr class="text-center" style="background-color: <?= VIEW_COLOR ?>; color: #000;">
					<th class="text-center" style="width:4%; color: #000;">SR No.</th>
					<th class="image-width text-center" style="width:8%; color: #000;">Image</th>
					<th colspan="3" class="text-center" style="width:24%; color: #000;">Product Name</th>
					<th class="text-center" style="width:8%; color: #000;">Brand <br> Name</th>
					<th class="text-center" style="width:8%; color: #000;">HSN Code</th>
					<th class="text-center" style="width:5%; color: #000;">Qty</th>
					<th class="text-center" style="width:7%; color: #000;">Weight<br />(in kg)</th>
					<th class="text-center" style="width:7%; color: #000;">Rate</th>
					<th class="text-center" style="width:7%; color: #000;">Discount %</th>
					<th class="text-center" style="width:10%; color: #000;">Discounted Value</th>
					<th class="text-center" style="width:6%; color: #000;">GST /<br>IGST %</th>
					<th class="text-center" style="width:11%; color: #000;">Total Amount</th>
				</tr>
				<?php
				$ITEMS = array();
				...
				?>
						<tr class="product-item-row">
							...
							<td class="text-center"><?php echo $db->rp_getValue("order_item_brand_master", "name", "isDelete=0 AND isActive=1 AND id='" . $item['order_item_brand_id'] . "'") ?></td>
							<td class="text-center"> <?= $hsncode ?></td>
							<td class="text-center"><?= $qty_product;
							...
							<td class="text-center"><?php echo round($item_discount_per, 2); ?></td>
							<td class="text-center"><?php echo $currency . ' ' . round($item_discount_total, 2); ?></td>
							<td class="text-center"><?= $pro_gst = $db->rp_getValue("product", "igst", "id='" . $item['pro_id'] . "' AND isDelete=0", 0); ?></td>
							<td class="text-center"><?php echo $currency . ' ' . round($item['totalprice'], 2); ?></td>
						</tr>
						<?php
					}
				}
				...
				<tr>
					...
					<td style="text-align: center"><strong><?php echo $weight_total; ?></strong></td>
					<td></td>
					<td></td>
					<td></td>
					<td></td>
...
?>
	...
	<?php armor_quotation_pi_echo_suggest_block_for_order($db, $order_id, $orderViewEmbedded); ?>
	</div>
	<div class="quote-summary-body">
		<table class="quote-table quote-summary-terms-table">
			<tbody>
				<tr class="font-size">
					<td colspan="16" class="quote-summary-terms-cell" style="background-color: lightgray;">
			...
			<tbody class="<?= $cl; ?>">
				<tr class="font-size quote-summary-details-row">
					<td colspan="8" class="quote-summary-info-cell" valign="top">
						<span class="font-13"><b>Bill Amount In Words</b> :
							<?php
						...
						<span class="font-13"><b>Note</b> <?php echo $cart_detail_d['remarks'] ?></span><br>
						...
						?>
						<br /><span style="color: red;">Edited By : <?= $modified_by_name ?> &nbsp; </span><br>
						<span style="background-color: #FFFF33;"><b>Note : KINDLY RELEASE PAYMENT FOR DISPATCH CLEARANCE</b></span>
					</td>
				...
				<?php if ($cart_detail_d['tcs_amount'] != "" && $cart_detail_d['tcs_amount'] != "0") { ?>
					<tr>
						<td colspan="4"><strong>TCS (<?= TCS_CHARGE_IN_PER ?>%)</strong></td>
						<td colspan="4" class="text-right"><strong><?= $currency . number_format($cart_detail_d['tcs_amount'], 2) ?></strong></td>
					</tr>
				<?php } ?>
				<tr>
					<td colspan="4"><strong>Round Off</strong></td>
					<td colspan="4" class="text-right"><strong><?php echo $currency . $display_roundoff; ?></strong></td>
				</tr>
				<tr style="background-color: <?= GRAND_TOTAL_COLOR ?>;font-size: 16px;">
					<td colspan="4"><strong>Grand Total</strong></td>
					<td colspan="4" class="text-right"><strong><?php echo $currency . ' ' . $db->rp_number_format($display_grand_total, 2); ?></strong></td>
				</tr>
							</tbody>
						</table>
					</td>
	...
	<div class="quote-footer-wrap">
		...
		<table class="quote-table quote-footer-table">
			<tbody>
				<tr>
					<td class="quote-footer-cell" colspan="16">
						...
						?>
							<img class="quote-footer-img" src="../images/white_footer.jpg" alt="Footer">
						<?php
						}
						?>
					</td>
...
<div class="quote-print-toolbar">
	Pro Forma Invoice ready — Items, Suggested Products, Terms & Footer
	<button type="button" onclick="orderPrintNow();">Print Pro Forma Invoice</button>
</div>
<?php } ?>
<?php if ($orderViewStandalone) { ?>
<script type="text/javascript">
(function() {
	var orderPrintTitle = <?= json_encode($orderPrintTitle) ?>;

	window.orderPrintNow = function() {
