...
<link rel="stylesheet" type="text/css" href="print_css.css">
</head>
<body>
<!-- for original -->
	...
	<div class="page-header">
      <table width="100%" > 
		...
		<!-- <tr>
			...
			<td colspan="3" class="no-border-left no-border-bottom text-left border-r-width border-gray" ><span><img style="margin: 0px 0px 0px 0px;" width="310" height="180px;"  src="<?= SITEURL.VIEW_LOGO ?>"></span></td>

			<td colspan="13" class="no-border-bottom " style="font-size: 27px; padding-left: 20px;"><strong style="font-size: 35px;"><?php echo CLIENT_NAME ?></strong><br>
			<p class="font-13"><?= CLIENT_ADDRESS ?> | <?= OFFICE_PHONE ?><?= OFFICE_EMAIL ?> | <?= OFFICE_WEBSITE ?> <br> <strong>GST No : </strong><?= COMPANY_GST ?></p>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<td colspan="5" class="no-border-top no-border-right height-5 text-center"></td>
			<td colspan="6" class="no-border-top no-border-right height-5 text-center"><b>GST - <?= COMPANY_GST ?></b></td>
			<td colspan="5" class="no-border-top height-5 text-center"></td>
		</tr>

		<tr>
			<td colspan="5" class="no-border-bottom height-5 text-center"></td>
			<td colspan="6" class="no-border-bottom no-border-left height-5 text-center"><b>PAN - <?= COMPANY_PAN ?></b></td>
			<td colspan="5" class="no-border-bottom no-border-left height-5 text-center"></td>
		</tr>

		<tr>
			<td colspan="5" class="no-border-right no-border-bottom bg-gray"><b>Debit Memo</b></td>
			<td colspan="6" class="text-center no-border-right no-border-bottom bg-gray"><b>Tax Invoice</b></td>
			<td colspan="5" class="text-right no-border-bottom bg-gray headtitle"><b><?=$title?></b></td>
		</tr>
		<tr>
			<td colspan="16" class="no-border-bottom no-border-top height-5"></td>
		</tr>
		<tr class="vertical-top">
			<td colspan="5" class="no-border-bottom no-border-top border-r-width">
				...
				<span>Contact Person : <?=$cart_detail_d['customer_name']?></span><br>
				<span>Contact Number : <?=$cart_detail_d['contact_number']?></span><br>
				<span>Email : <?=$cart_detail_d['email']?></span><br/>
				<span>State of Supply : <?=$cart_detail_d['state']?></span><br/>
				<strong><span>GST No : <?=$cart_detail_d['gst']?></span></strong><br/>
				<span><b>Pan No :</b> <?=$cart_detail_d['pan']?><br/>
				<span>Vendor Code : <?= $cart_detail_d['vendor_code'] ?></span><br/>
				<!-- <span>Mobile : <?=$cart_detail_d['phone']?></span><br/> -->
				<!-- <span>Date:- :</span><br/> -->
			</td>
			...
			<td colspan="5" class="no-border-bottom no-border-top ">
				...
				<table class="no-border-top no-border-bottom no-border-right no-border-left" style="width: 45mm!important;margin:0px;">
					<tr>
					...
					<tr>
						<td class="no-border-right no-border-bottom no-border-left">P.O. No : <?= $cart_detail_d['po_no']?></td>
					</tr>
					<tr>
						<td class="no-border-right no-border-bottom no-border-left">P.O. Date : <?php if($cart_detail_d['po_date']!="0000-00-00 00:00:00"){ echo date('d-m-Y',strtotime($cart_detail_d['po_date']));}else{ echo ""; }?></td>
					</tr>
					<!-- <tr>	
						<td class="no-border-right no-border-bottom no-border-left"><b>Challan No :</b> <?= $cart_detail_d['chalan_no'] ?></td>
					</tr> -->
					<tr>
						<td class="no-border-right no-border-bottom no-border-left">E-Way Bill No : <?= $cart_detail_d['way_bill_no'] ?></td></tr>
					<tr>	
						<td class="no-border-right no-border-bottom no-border-left">Warehouse : <br/> <?= $name ?></td>
					</tr>
					<tr>	
						<td class="no-border-right no-border-bottom no-border-left">LUT No : <?= $cart_detail_d['lut_no'] ?></td>
					</tr>
  ...
  <div class="page-footer">
    <table>
    	<tr class="font-size">			
			<td colspan="4" class="" rowspan="11" style="vertical-align: top;">	 
				<?php
					$subtotal1 = $db->rp_getValue("invoice_new_product_item","SUM(pro_qty*unitprice)","isDelete=0 AND invoice_id='".$invoice_id."' AND dispatch_item_type=1",0);
					$final_price1 = $subtotal1-$cart_detail_d['cash_discount_amount'];
					...
					$gst_amount=$cart_detail_d['igst_amount'];
			...
			<td colspan="4" rowspan="11" style="vertical-align: top;">
			...
			<td colspan="5" class="text-right font-13"><strong><?php echo CURR.' '.$db->rp_number_format(round($subtotal1,2),2); ?></strong></td>
		</tr>
		...
		<tr>
			...
			<td colspan="5" class="text-right font-13"><strong><?php echo CURR.' '.$db->rp_number_format(round($cart_detail_d['transport_charge'],2),2); ?></strong></td>
		</tr>

		<tr>
			<td colspan="3" class="text-left font-13"><strong>Packing & Forwarding Charge</strong></td>			
			<td colspan="5" class="text-right font-13"><strong><?php echo CURR.' '.$db->rp_number_format(round($cart_detail_d['packing_charge'],2),2); ?></strong></td>
		</tr>

		<tr>
			<?php
			$total_tax_amt = $subtotal1 - ($cart_detail_d['cash_discount_amount'] + $cart_detail_d['additional_discount_amount']) + ($cart_detail_d['transport_charge']+$cart_detail_d['packing_charge']);
			?>
			<td colspan="3" class="text-left font-13"><strong>Total Taxable Amount</strong></td>
			<td colspan="5" class="text-right font-13"><strong><?php echo CURR.' '.$db->rp_number_format(round($total_tax_amt,2),2); ?></strong></td>
		</tr>
		...
		if($cart_detail_d['igst_amount']!="0")
		{
			if($Distri_data_a['type_of_executive']==7)
			{
					if (strtolower(CLIENT_STATE) == strtolower($Distri_data_a['state'])) 
					{
						?>
							<tr>
								<td colspan="3" class="text-left"><strong>C GST</strong></td>			
								<td colspan="4" class="text-right "><strong><?= (CURR. $db->rp_number_format($cart_detail_d['igst_amount']/2,2))?></strong> </td>
							</tr>
							<tr>
								<td colspan="3" class="text-left"><strong>S GST</strong></td>			
								<td colspan="4" class="text-right "><strong><?= (CURR. $db->rp_number_format($cart_detail_d['igst_amount']/2,2))?></strong></td>
							</tr>
						<?php
					}
					else
					{
							?>
							<tr>
								<td colspan="3" class="text-left"><strong>IGST</strong></td>		
		...
		<tr style="font-size: 16px;">
			...
			<td colspan="5" class="text-right"><strong><?php echo CURR.' '.$db->rp_number_format(round($new_final_price,2),2); ?></strong></td>
		</tr>
		...
		<table>
			...
			<?php
				$ITEMS=array();
				$items1=$db->rp_getData("invoice_new_product_item","*","isDelete=0 AND invoice_id='".$invoice_id."' GROUP BY hsn_code","",0);
				...
				while ($item=mysqli_fetch_assoc($items1)) 
				{
					...
					if($items1)
					{
						...
						$Invoice=$db->rp_getData("invoice_new_product_item","*","isDelete=0 AND invoice_id='".$invoice_id."' AND hsn_code='".$item['hsn_code']."'","",0);
						$InvoiceIds = array();
						...
						$total_pro_qty=$db->rp_getValue("invoice_new_product_item","SUM(pro_qty)","id In (".$InvoiceIds.") AND isDelete=0",0);
						$total_pro_taxable=$db->rp_getValue("invoice_new_product_item","SUM(taxable)","id In (".$InvoiceIds.") AND isDelete=0",0);
						...
						if($cart_detail_d['igst_amount']!=0)
						{
							$gst_per_amount+=$db->rp_getValue("invoice_new_product_item","SUM(igst_amount)","id In (".$InvoiceIds.") AND isDelete=0",0); 
							if (strtolower(CLIENT_STATE) == strtolower($Distri_data_a['state'])) 
							{
				...
				<tr >
					...
					<!-- <?php 
					$Total_gst_per_amount = ($cart_detail_d['igst_amount'])?$Total_gst_per_amount:"";
					$Total_cgst_per_amount = ($cart_detail_d['igst_amount'] && $Total_cgst_per_amount>0)?$Total_cgst_per_amount:"";
					$Total_sgst_per_amount = ($cart_detail_d['igst_amount'] && $Total_cgst_per_amount>0)?$Total_cgst_per_amount:"";
					?> -->
	...
	<!-- hsn summary -->
		...
		<tr>
			...
			<td colspan="5" rowspan="4" class="no-border-right">
				<br><br><br><br>
				<center><strong style="margin-left: -350px;">Checked By</strong></center>	
			</td>
		...
		<!-- <tr>
			<td colspan="8" style="border-top: none;border-right: none;">
				<span class="font-13"><br/><b>Terms & Condition :</b></span><br>
				<span style="font-size:13px;">1) We undertake no responsibility for breakage, shortage in transit of our paying useful attention to the dispatch.</span><br>
				<span style="font-size:13px;">2. Our risk & responsibility ceases as soon as goods leaves our premises.</span><br/>
				<span style="font-size:13px;">3. Goods once sold will not be accepted back.</span><br>
				<span style="font-size:13px;">4. Interest @1.5% Per Month will be charged,
					If invoice amount is not paid on or before due date.(As per MSME ACT 2006)
				</span><br>
				<span style="font-size:13px;">5. Subject to RAJKOT jurisdiction only, E.&.O.E</span>
			</td>
			<td colspan="8">
				<center><strong>For, <?= CLIENT_BRAND_NAME ?><br><img   src="<?= SITEURL.VIEW_STAMP ?>" height=119px;>.<br>
				Authorised SIgnatory</strong></center>	
			</td>
		</tr> -->
    </table>
...
<table style="page-break-after: always!important;">
	...
	<tbody class="<?= $cl; ?> page" style="border:1px solid #000!important">
		
		<tr class="text-center tdmr">
			...
			<th colspan="1" class="text-center">GST Rate</th>
		...
		$totalRecord = $db->rp_getTotalRecord("invoice_new_product_item","invoice_id='".$invoice_id."' AND dispatch_item_type=1");
		...
		if($items1)
		{
			...
			<tr>
				...
				<td colspan="3" class="text-center rate"><?php echo CURR.' '.round($item['pro_qty']*$item['unitprice'], 2); ?></td>
				 ...
				 <?php
			      if($count%10 == 0 && $count!=$totalRecord)
			      {
			      ...
			      ?>
						   ...
						   <table cellpadding="0" cellspacing="0" class="first" style="page-break-after: always;"> 
						  	<tbody style="border:1px solid #000!important">
						  		<tr><td colspan="15" style="height: 535">&nbsp;</td></tr>
						  		  <tr>
						      	<th colspan="1" class="srno text-center" >SR</th>
										<th colspan="4" class="pname model text-center">Item Description</th>
										<th colspan="2" class="text-center" style="width:10%!important;">Product Code</th>
