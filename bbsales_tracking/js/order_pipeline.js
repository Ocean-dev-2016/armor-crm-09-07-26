        ...
        success: function(result) {
            ...
            if (result.a == 1) {
                $.each(result.result, function(i, Task) {
                    CreateTaskCard(Task.total_order,Task.id,Task.company_name,Task.quotation_no,Task.grand_total,Task.order_date,Task.sales_id,Task.inquiry_id,Task.contact_number,Task.customer_type,Task.isActive,Task.customer_id,Task.customer_name,Task.sales_executive_name,Task.country,Task.state,Task.city,Task.status,Task.order_no);
                });
...
function CreateTaskCard(total_order=0,TaskID,company_name,quotation_no,grand_total,order_date,sales_id,inquiry_id,contact_number,customer_type,IsActive,customer_id,customer_name,sales_executive_name,country,state,city,status,order_no,TaskPriority=1,taskcolor="green",TaskBy=1,TaskName="",TaskAssignedBy="") {
        
    var Task_date = $.datepicker.formatDate("dd-M-yy", new Date(order_date));
    if (CurrentView == 0) {
        var icon = "";
        var taskcolor = "";
        var edit_btn = "";
        ...
        {
            var edit_btn = '<li><a data-id="' + TaskID + '" target="_blank" href="orders_crud.php?mode=edit&id=' + TaskID + '" class="pipeline-edit-btn" title="Edit"><i class="fa fa-edit"></i>&nbsp;Edit Order</a></li>';
        }

        if(status==1)
        {
            var dispatch_btn = '<li><a data-id="' + TaskID + '" target="_blank" href="dispatch_crud.php?mode=add&order_id=' + TaskID + '" class="pipeline-edit-btn" title="Edit"><i class="fa fa-edit"></i>&nbsp;Dispatch</a></li>';
        }

        /*if(status==2)
        {
            var download_btn = '<li><a data-id="' + TaskID + '" target="_blank" href="order_viewer.php?mode=add&order_id=' + TaskID + '" class="pipeline-edit-btn" title="Edit"><i class="fa fa-edit"></i>&nbsp;Dispatch</a></li>';
        }*/

        var TaskCard = ' <li class="task-card" data-priority="' + TaskPriority + '" data-status="' + status + '" id="TaskCard' + TaskID + '" data-id="' + TaskID + '" style="min-height:215px!important; border-left-color:' + taskcolor + '" data-href="order_viewer.php?order_id=' + TaskID + '">' +
            '<div class="task-card-title">' +
            '<span class="open-task" style="color:black">' + icon + " " + company_name + "</span></br>" +
            '<span class="open-task" style="color:black;font-size:16px;">' + total_order + "</span></br>" +
            '<span class="open-task" style="color:black;font-size:10px;">' + Task_date + " - " + quotation_no + "</span></br>" +
            ...
            '<li><a data-id="' + TaskID + '" target="_blank" href="order_viewer.php?&order_id=' + TaskID + '" class="task-view-btn"><i class="fa fa-eye"></i>&nbsp;Download</a></li>' +
            edit_btn + 
            dispatch_btn + 
           '</ul>' +
            '</div> ' +
            ...
            "</u></b></span>&nbsp;<br/><b><span style='font-size:8px;float:left'> - " + country + " - " + state + " - " + city + " </span>&nbsp;" +
            
            "</u></b></span>&nbsp;<br/><b><span style='font-size:10px;float:left'> - " + order_no + "</span>&nbsp;" +
...
function UpdateTaskStatus(TaskID, NewStatus) {
    ...
    {
        toastr.error("You can Not Create Direct Dispatch Or Partially Dispatch From Here.");
        alert("You can Not Create Direct Dispatch Or Partially Dispatch From Here.");
        location.reload();
    }
    else
    {
