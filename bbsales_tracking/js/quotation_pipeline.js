        ...
        success: function(result) {
            ...
            if (result.a == 1) {
                $.each(result.result, function(i, Task) {
                    CreateTaskCard(Task.total_quotation,Task.id,Task.company_name,Task.quotation_no,Task.grand_total,Task.quotation_date,Task.sales_id,Task.inquiry_id,Task.contact_number,Task.customer_type,Task.isActive,Task.customer_id,Task.customer_name,Task.sales_executive_name,Task.country,Task.state,Task.city,Task.status);
                });
...
function CreateTaskCard(total_quotation=0,TaskID,company_name,quotation_no,grand_total,quotation_date,sales_id,inquiry_id,contact_number,customer_type,IsActive,customer_id,customer_name,sales_executive_name,country,state,city,status,TaskPriority=1,taskcolor="green",TaskBy=1,TaskName="",TaskAssignedBy="") {
    var Task_date = $.datepicker.formatDate("dd-M-yy", new Date(quotation_date));
    if (CurrentView == 0) {
        var icon = "";
        var taskcolor = "";
        var edit_btn = "";
        ...
        if(status==0 || status==1 || status==4 || status==5 || status=='-2' || status==3)
        {
            var edit_btn = '<li><a data-id="' + TaskID + '" target="_blank" href="quotation_crud.php?mode=edit&id=' + TaskID + '" class="pipeline-edit-btn" title="Edit"><i class="fa fa-edit"></i>&nbsp;Edit Quotation</a></li>';

            var followup_btn = '<li><a data-id="' + TaskID + '" target="_blank" href="followup.php?mode=quotation_followup&quotation_id=' + TaskID + '" class="pipeline-edit-btn" title="Edit"><i class="fa fa-edit"></i>&nbsp;Quotation Followup</a></li>';
        }

        if(status==0 || status==1 || status=='-2' || status==3)
        {
            var lost_btn = '<li><a class="" data-toggle="modal" data-target="#lostModal" onclick="LostAdd('+TaskID+');" class="pipeline-edit-btn" title="Edit"><i class="fa fa-chain-broken"></i>&nbsp;Lost</a></li>';
        }

        if(status==1)
        {
            var revised_btn = '<li><a data-id="' + TaskID + '" target="_blank" href="quotation_crud.php?mode=add&quotation_id=' + TaskID + '" class="pipeline-edit-btn" title="Edit"><i class="fa fa-edit"></i>&nbsp;Revised Quotation</a></li>';

            var gen_order_btn = '<li><a onclick="GenerateOrder('+TaskID+')" title="Generate Order"><i class="fa fa-shopping-cart"></i>&nbsp;Generate Order</a></li>';
        }

        var TaskCard = ' <li class="task-card" data-priority="' + TaskPriority + '" data-status="' + status + '" id="TaskCard' + TaskID + '" data-id="' + TaskID + '" style="min-height:220px!important; border-left-color:' + taskcolor + '" data-href="quotation_viewer.php?quotation_id=' + TaskID + '">' +
            '<div class="task-card-title">' +
            '<span class="open-task" style="color:black">' + icon + " " + company_name + "</span></br>" +
            '<span class="open-task" style="color:black;font-size:16px;">' + total_quotation + "</span></br>" +
            '<span class="open-task" style="color:black;font-size:10px;">' + Task_date + " - #INQ " + inquiry_id + "</span></br>" +
            '<span class="open-task" style="color:black;font-size:10px;">' + sales_executive_name + "</span>" +
            '<div class="dropdown" style="margin-top:-30px;">' +
            '<a class="btn-task-card-more dropdown-toggle" data-toggle="dropdown">' +
            '<i class="fa fa-ellipsis-v"></i>' +
            '</a>' +
            '<ul class="dropdown-menu">' +
            '<li><a data-id="' + TaskID + '" target="_blank" href="quotation_viewer.php?&quotation_id=' + TaskID + '" class="task-view-btn"><i class="fa fa-eye"></i>&nbsp;Download</a></li>' +
            edit_btn + 
            lost_btn + 
            revised_btn + 
            gen_order_btn + 
           ...
           '</ul>' +
            ...
            '<div class="" style="font-size:12px;width:100%;float:left"><span style="float:left"> <b><u>' + customer_name +
            ...
            "</u></b></span>&nbsp;<br/><b><span style='font-size:10px;float:left'> - " + quotation_no + "</span>&nbsp;" +
            
            "</u></b></span>&nbsp;<br/><b><span style='font-size:10px;float:left'> - " + contact_number + "</span>&nbsp;" +
            '</div>' +
            '</li>';
        ...
        var TaskCard = ' <tr class="task-card" data-priority="' + TaskPriority + '" data-status="' + CurrentTaskStatus + '" id="TaskCard' + TaskID + '" data-id="' + TaskID + '" style="border-left-color:' + taskcolor + ';  background-color:' + deadlineColor + '; color:' + deadlineColorfont + ';" data-href="sales_pipeline_edit.php?mode=view&id=' + TaskID + '">' +
            '<td>' +
            '<span class="open-task">' + quotation_no + "</span>" +
            '</td>' +
            '<td class="open-task">' +
            "" + quotation_date + "" +
            '</td>' +
