<?php
$page_id=660;$page_slug='self_analysis_master';
$ctable 	= "self_analysis_master";
$ctable1 	= "Self Analysis Question Master";
...
include("connect.php");
require_once("../include/class.self_analysis_master.php");
$objSelfAnalysis= new SelfAnalysis();
$questions			= "";

if(isset($_REQUEST['submit'])){
	// print_r($_REQUEST);exit;
	$detail['questions']					= $db->clean($_REQUEST['questions']);
	...
	if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="add"){
		...
		$reply=$objSelfAnalysis->InsertSelfAnalysis($detail);
		if($reply['ack']==1)
		{
			$db->addSuccessMessage($reply['ack_msg']);
			$db->rp_location("self_analysis_que_manage.php?msg=inserted");
		}else{
	...
	else if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="edit")
	{
		...
		$reply=$objSelfAnalysis->UpdateSelfAnalysis($detail);
		if($reply['ack']==1){
			$db->addSuccessMessage($reply['ack_msg']);
		    $db->rp_location("self_analysis_que_manage.php?msg=updated");
		}
		else{
				 $db->addErrorMessage($reply['ack_msg']);
			} 
...
if(isset($_REQUEST['id']) && $_REQUEST['id']>0 && $_REQUEST['mode']=="edit"){
	if($rights['update_flag']!=1)
		...
		$reply=$objSelfAnalysis->GetEditDataCompany($detail);
		if($reply['ack']==1){
...
if(isset($_REQUEST['id']) && $_REQUEST['id']>0 && $_REQUEST['mode']=="delete"){
	if($rights['delete_flag']!=1)
		...
		$detail['id']=$_REQUEST['id'];
		$reply=$objSelfAnalysis->DeleteSelfAnalysis($detail);
		if($reply['ack']==1){
		$db->addSuccessMessage($reply['ack_msg']);
		$db->rp_location("self_analysis_que_manage.php?msg=inserted");
		}
						...
						<div class="portlet-body form">
							...
							<div class="row">
								...
								<div class="form-group">
									<label>Questions <code>*</code></label>
									<input type="text" class="form-control" name="questions" id="questions" value="<?php echo $questions; ?>">
							...
							<div class="form-actions">
								...
								<button type="button" class="btn btn-default" onClick="window.location.href='self_analysis_que_manage.php'">Back</button>
							</div>
						</div>
					</div>
				</div>
