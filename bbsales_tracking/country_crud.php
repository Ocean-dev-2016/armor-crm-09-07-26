...
$page_title = ucwords($mode)." "."Country";
$page_hierarchy=array(array("link"=>"","title"=>"Master"),array("link"=>$ctable."_manage.php","title"=>"Manage Country"),array("link"=>$ctable."_crud.php","title"=>"Add/Edit Country"));
include("connect.php");
// echo "string";exit();
require_once("../include/class.country.php");
...
if(isset($_REQUEST['id']) && $_REQUEST['id']>0 && $_REQUEST['mode']=="delete"){
	...
	$reply=$objClassType->ClassTypeDelete($detail);
		if($reply['ack']==1){
		$db->addSuccessMessage($reply['ack_msg']);
		$db->rp_location($ctable."_manage.php?msg=delete");
		}
		else{
			$db->addErrorMessage($reply['ack_msg']);
		}
...
function check_form(){
	...
	if($("#name").val()=="" || $("#name").val().split(" ").join("")==""){		
		vd=aj.error('name',"Please Enter Country Name.","add_error");
		isValid=false;
	}
