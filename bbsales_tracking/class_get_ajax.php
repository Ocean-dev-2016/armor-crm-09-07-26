...
else
{
	...
	{
	 $ctable_where .= "  country_id = '".$_REQUEST['country_id']."' AND ";
	...
	<table id="datatable_sales_executive_type" class="table table-striped table-bordered table-hover">
        <thead class="fix-th">
        	<tr>
                ...
                <th></th>
                <th> 
                	<select class="form-control input-medium status" name="country_id" id="country_id" >
                                            <option value="">Select Country</option>  
                                            <option value="all">all</option>  
                     <?php 
                        $country_list_r=$db->rp_getData('country',"*","isDelete=0","",0);
                        while($country_list_d=mysqli_fetch_assoc($country_list_r))
                        {
                            ?>
                                            <option <?php echo ($country_id==$country_list_d['id'])?"selected":"" ; ?> value="<?php echo $country_list_d['id']?>">
                                            <?php echo $country_list_d['name'];?>
                                            </option>
                            <?php
                        }
                    ?>
        ...
        {
        	?>
        	<tr><td colspan="4" class="text-center"><h4><strong>Use filter for the show data of State.</strong> </h4></td></tr>
        	<?php
        }
        ?>
        </tbody>
