<?php
$page_id=664;$page_slug='terms_condition';
$ctable     = "terms_condition";
$ctable1     = "Terms & Condition";
...
include("connect.php");
require_once("../include/class.terms_condition.php");
...
if (isset($_REQUEST['submit'])) {
    ...
    if (isset($_REQUEST['mode']) && $_REQUEST['mode'] == "add") {
        ...
        $reply = $objDesignation->InsertData($detail);
        if ($reply['ack'] == 1) {
        ...
        $reply = $objDesignation->UpdateData($detail);
        if ($reply['ack'] == 1) {
            $db->addSuccessMessage($reply['ack_msg']);
...
if (isset($_REQUEST['id']) && $_REQUEST['id'] > 0 && $_REQUEST['mode'] == "edit") {
    ...
    $reply = $objDesignation->GetEditData1($detail);
    if ($reply['ack'] == 1) {
...
if (isset($_REQUEST['id']) && $_REQUEST['id'] > 0 && $_REQUEST['mode'] == "delete") {
    ...
    $detail['id'] = $_REQUEST['id'];
    $reply = $objDesignation->DeleteData($detail);
    if ($reply['ack'] == 1) {
        $db->addSuccessMessage($reply['ack_msg']);
        $db->rp_location($ctable . "_manage.php?msg=inserted");
    } else {
                                        ...
                                        <div class="row">
                                            <div class="col-md-12">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <textarea type="text" class="form-control" id="description" name="description" rows="3" cols="60"><?php echo str_replace('rn','',$description); ?></textarea>
                                                    <p class="help-block"></p>
                                                </div>
                                            </div>
                                        </div>
       ...
       CKEDITOR.replace('description');
        ...
        function check_form() {
            ...
            if ($("#description").val() == "" || $("#description").val().split(" ").join("") == "") {
                vd = aj.error('description', "Please Enter Terms & Condition.", "add_error");
                isValid = false;
            }
            if (isValid) {
                return true;
