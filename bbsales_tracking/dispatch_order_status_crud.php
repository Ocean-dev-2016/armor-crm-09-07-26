<?php
$page_id=625;$page_slug='page_tax';
$ctable 	= "dispatch_order_status";
$ctable1 	= "Dispatch Order Status";
...
include("connect.php");
require_once("../include/dispatch_order_status.class.php");
$objUnit= new Unit();
$name			= "";
$code			= "";
if(isset($_REQUEST['submit'])){
	...
	if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="add"){
		...
		$reply=$objUnit->InsertUnit($detail);
		if($reply['ack']==1)
		{
	...
	else if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="edit")
	{
		...
		$reply=$objUnit->UpdateUnit($detail);
		if($reply['ack']==1){
			$db->addSuccessMessage($reply['ack_msg']);
...
if(isset($_REQUEST['id']) && $_REQUEST['id']>0 && $_REQUEST['mode']=="delete"){
	...
	$detail['id']=$_REQUEST['id'];
	$reply=$objUnit->DeleteUnit($detail);
		if($reply['ack']==1){
		$db->addSuccessMessage($reply['ack_msg']);
		$db->rp_location($ctable."_manage.php?msg=inserted");
		}
