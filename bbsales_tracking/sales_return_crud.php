...
$page_slug  = 'add_sales_return';
$ctable     = "sales_return";
$ctable1    = "Sales Return";
$page_title = ucwords($_REQUEST['mode']) . " " . $ctable1;
$page_hierarchy=array(array("link"=>"","title"=>"Sales & Marketing"),array("link"=>"sales_return_manage.php","title"=>$ctable1),array("link"=>$ctable1."_crud.php","title"=>"Add/Edit ".$ctable1));
include("connect.php");
include("../include/class.sales_return.php");
$obj = new SalesReturn();
...
if (isset($_REQUEST['submit'])) 
{
	...
	$detail['original_invoice_id'] = implode(",", $_REQUEST['original_invoice_id']);
	// $original_invoice_id = implode(",", $_REQUEST['original_invoice_id']); 
	if ($detail['original_invoice_id'] == "") 
	{
		// $original_invoice_id ="";
	 	$detail['original_invoice_id'] = "";
	}
	...
	if (isset($_REQUEST['mode']) && $_REQUEST['mode'] == "add") 
	{
		...
		$reply = $obj->AddInvocie($detail, $item);
		if ($reply['ack'] == 1) 
		{
			...
			$obj->PlaceInvociePanel($detail);

			$customer_id = $db->rp_getValue("sales_return","customer_id","id='".$reply['order_id']."'",0);
			 
			$db->addSuccessMessage($reply['ack_msg']);
			$db->rp_location("sales_return_manage.php");
			$db->rp_location("sales_return_manage.php?msg=inserted");
		} 
		else 
		{
	...
	else if (isset($_REQUEST['mode']) && $_REQUEST['mode'] == "edit") 
	{
		...
		$detail['round_off']		  = isset($_REQUEST['round_off'])?$db->clean($_REQUEST['round_off']):"";
		$reply = $obj->UpdateInvoice($detail, $item);
		if ($reply['ack'] == 1) 
		{
		...
		{
			$db->addErrorMessage($reply['ack_msg']);
			$db->rp_location("sales_return_crud.php?mode=edit&id=".$_REQUEST['id']);
		}
	}	
}	
...
if (isset($_REQUEST['id']) && $_REQUEST['id'] > 0 && $_REQUEST['mode'] == "edit") {
	...
	$OrderNo = $db->rp_getValue("sales_return", "order_no", "id=" . $_REQUEST['id'] . " AND isDelete=0", 0);
	$Order = $db->rp_getValue("sales_return", "customer_id", "id=" . $_REQUEST['id'] . " AND isDelete=0", 0);
	$customer_name = $db->rp_getValue("customer", "company_name", "id=" . $Order . " AND isDelete=0", 0);
	$page_title = ucwords($_REQUEST['mode']) . '&nbsp' . "Sales Return";
	$reply = $obj->GetOrder($detail);
	$item_info = $obj->GetOrderItems($detail);
	if ($reply['ack'] == 1) 
	{
		$id = $_REQUEST['id'];
		$result = $reply['result'];
...
if (isset($_REQUEST['id']) && $_REQUEST['id'] > 0 && $_REQUEST['mode'] == "delete") 
{
	...
	/*log entry*/
	$customer_id = $db->rp_getValue("sales_return","customer_id","id='".$_REQUEST['id']."'");
	$invoice_no = $db->rp_getValue("sales_return","invoice_no","id='".$_REQUEST['id']."'");
	...
	$db->rp_update("sales_return",$rows,"id='".$_REQUEST['id']."'",0,$log_description,$flag,$module_name,"",$customer_id);
	$db->rp_update("sales_return_item",array("isDelete"=>1),"invoice_id='".$_REQUEST['id']."'");
	  
	$reply['ack'] = 1;
	$reply['ack_msg'] = "Sales Return Delete successfully..";
	if ($reply['ack'] == 1) 
	{
		$db->addSuccessMessage($reply['ack_msg']); 
		$db->rp_location("sales_return_manage.php");
		...
		<div class="col-md-12 col-sm-12 col-xs-12 portlet box grey-cascade box">
			<div class="portlet-title">
				<div class="caption">
					...
					<span class="caption-subject bold uppercase"> Sales Return DETAIL</span>
				</div>
			</div>
		</div>
		<div class="row">
						...
						<div class="row">
							...
							<div class="col-md-4 col-sm-4">
								...
								<div class="form-group">
									...
									?>
									<select class="form-control customer_id_s" name="customer_id" placeholder="Select Customer" id="customer_id"  type="text" <?php echo $disabled; ?> onChange="getinvoice(this.value)">
							...
							<div class="col-md-4 col-sm-4">
								<div class="col-md-6">
									<div class="form-group">
											...
											<select class="form-control" id="original_invoice_id" name="original_invoice_id[]" multiple>
							...
							<div class="col-md-4 col-sm-4">
								<div class="col-md-6">
									<div class="form-group">
										<label>Credit Note No. <code>*</code></label>
								...
								<div class="col-md-6">
									<div class="form-group">
										<label>Credit Note Date <code>*</code></label>
...
if (isValid) 
{
var r = confirm("Are You sure want to Save this Sales Return??");
if (r) 
{
return true;
} 
...
function getinvoice(cid,invoice_ids="") 
{
// $('#customer_id').select2("val", "");
invoice_ids='<?=$original_invoice_id?>';
$.ajax({
type: "post",
url: "get_customer_invoice_ajax.php",
data: "customer_id=" + cid +'&invoice_id=' + invoice_ids,
beforeSend: function() {
// $("#loading-modal").modal('show');
// $('.preloader').fadeIn('slow');
},
success: function(result){
	...
	$('#original_invoice_id').fSelect('destroy');
	
	$('#original_invoice_id').html(result);
	$('#original_invoice_id').fSelect('create');
}
