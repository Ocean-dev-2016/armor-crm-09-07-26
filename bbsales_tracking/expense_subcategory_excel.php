...
$file_name  = "Expence Sub Category Report".date('d_m_Y')."_".strtotime("now").".xlsx";
$Where = "";
...
$ctable1_r = $db->rp_getData("expence_sub_category","*",$ctable_where,"id DESC",0);
...
$objPHPExcel->getActiveSheet()->setCellValue($column1.$rowCount, "  Expence Sub Category");
$objPHPExcel->getActiveSheet()->setCellValue($column2.$rowCount, "Expence Category");
$objPHPExcel->getActiveSheet()->setCellValue($column3.$rowCount, "Sales Excutive");
...
{  
  ...
  for($j=0; $j<4;$j++)  
  {
    ...
    {
      $value =$db->rp_getValue("expence_category","name","id='".$row['expense_category_id']."'"); 
    }
    else if($j==4)
    {
      $value = $row['phone'];
    ...
    {
      $value = $db->rp_getValue("sales_executive","name","id IN('".$row['sales_executive_id']."')");;
    } 
    else if($j==6)
    {
      $value = $row['city'];
