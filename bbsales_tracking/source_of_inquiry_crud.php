<?php
$page_id=647;$page_slug='source_of_inquiry';
$ctable 	= "source_of_inquiry";
$ctable1 	= "Source Of Inquiry";
$main_page 	= "product_mgmt";
$page 		= "manage_".$ctable;
$page_title = ucwords($_REQUEST['mode'])." ".$ctable1;
$page_hierarchy=array(array("link"=>"","title"=>"Master"),array("link"=>"source_of_inquiry_manage.php","title"=>"Manage ".$ctable1),array("link"=>$ctable1."_crud.php","title"=>"Add/Edit ".$ctable1));
include("connect.php");
require_once("../include/source_of_inquiry.class.php");
$objInquiry= new SourceOfInquiry();
$name			= "";
$code			= "";
if(isset($_REQUEST['submit'])){
	...
	if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="add"){
		...
		$reply=$objInquiry->InsertWeight($detail);
		if($reply['ack']==1)
		{
	...
	else if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="edit")
	{
		...
		$reply=$objInquiry->UpdateWeight($detail);
		if($reply['ack']==1){
			$db->addSuccessMessage($reply['ack_msg']);
...
if(isset($_REQUEST['id']) && $_REQUEST['id']>0 && $_REQUEST['mode']=="edit"){
	if($rights['update_flag']!=1)
		...
		$reply=$objInquiry->GetEditDataWeight($detail);
		if($reply['ack']==1){
...
if(isset($_REQUEST['id']) && $_REQUEST['id']>0 && $_REQUEST['mode']=="delete"){
	if($rights['delete_flag']!=1)
		...
		$detail['id']=$_REQUEST['id'];
		$reply=$objInquiry->DeleteWeight($detail);
		if($reply['ack']==1){
		$db->addSuccessMessage($reply['ack_msg']);
		$db->rp_location($ctable."_manage.php?msg=inserted");
		}
						...
						<div class="portlet-body form">
							...
							<div class="row">
								...
								<div class="form-group">
									<label>Source Of Inquiry Name <code>*</code></label>
...
function check_form(){
	...
	if($("#name").val()=="" || $("#name").val().split(" ").join("")==""){
		...
		vd=aj.error('name',"Please Enter Source Of Inquiry Name.","add_error");
		isValid=false;
	}
	if(isValid)
	{
