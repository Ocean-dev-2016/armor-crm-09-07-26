...
$file_name  = "Lead_Source_Report"."_".date("d-m-Y").".xlsx";
...
$ctable1_r = $db->rp_getData("industry_type","*",$ctable_where,"id DESC",0);  
...
$column4  = 'E';
  ...
  $objPHPExcel->getActiveSheet()->setCellValue($column1.$rowCount, "Industry Type");
  $objPHPExcel->getActiveSheet()->setCellValue($column2.$rowCount, "Raw Data Count");
  $objPHPExcel->getActiveSheet()->setCellValue($column3.$rowCount, "Inquiry Count");
  $objPHPExcel->getActiveSheet()->setCellValue($column4.$rowCount, "Lead Count");
...
{  
  ...
  for($j=0; $j<5;$j++)  
  {
    ...
    {
      $value = $db->rp_getTotalRecord("no_order_inquiry","isDelete=0 AND industry_type_id='".$row['id']."' AND inquiry_type=-1",0);
    }

    else if($j==3)
    {
      $value = $db->rp_getTotalRecord("no_order_inquiry","isDelete=0 AND industry_type_id='".$row['id']."' AND inquiry_type=0",0);
    }
