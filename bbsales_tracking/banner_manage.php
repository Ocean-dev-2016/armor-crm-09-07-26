...
if(isset($_REQUEST['submit'])){
	...
	for($i=1;$i<=$disp_count;$i++){
		...
		if($b_id>0){
			...
			$check_disp=$db->rp_getTotalRecord("promotion","isDelete=0 AND display_order='".$display_order."' AND id!='".$b_id."'");
			if($check_disp >0)
			{
...
var searchName="";
var data_url = "banner_get_ajax.php";
...
function del_conf(id){
	...
	if(r){
		window.location.href='banner_crud.php?mode=delete&id='+id;
	}
}
...
function CheckDispalyOrder(id)
{
	...
	$.ajax({
		...
		data: 'display_order='+display_order+"&id="+size_id+"&table=promotion",
