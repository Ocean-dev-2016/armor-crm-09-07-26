<?php
$page_id=635;$page_slug='payment';
/*
 * @author Ravi Patel
 */
include("connect.php");
$ctable 	= "employee_payment";
$ctable1 	= "employee_payment";
$ctable_where = "";
// Get the total number of rows in the table
if(isset($_REQUEST['searchName']) && $_REQUEST['searchName']!=""){
	$customer_r=$db->rp_getData("sales_executive","id","name like '%".$_REQUEST['searchName']."%'","",0);
	// $customer_r=$db->rp_getData("sales_executive ","id","name like '%".$_REQUEST['searchName']."%'","",0);
	$cust_id=array();
	if($customer_r){
	...
	$ctable_where .="sales_id IN (".$cust_id.") AND";
	...
	<table id="datatable_1" class="table table-striped table-bordered table-hover">
        ...
        ?>
            <tr>
				...
				<?php $sales_name=$db->rp_getValue("sales_executive","name","id='".$ctable_d['sales_id']."'",0); ?>

				<td>
					<span class="<?php echo ($ctable_d['isActive']==0)?"text-danger":"text-success"; ?>"><?php echo stripslashes($sales_name); ?></span>
				</td>
...
<script type="text/javascript">
	...
	{   
        ...
        {
        	$.ajax({
                url:"employee_payment_ajax_function.php",
                type:"POST",
                data:{
                    m:mode,
                    id:id,                
