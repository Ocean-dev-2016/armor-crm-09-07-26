<?php
$page_id=641;$page_slug='page_followup_reason';
$ctable     = "followup_reason";
$ctable1     = "Followup Reason";
...
include("connect.php");
require_once("../include/class.followup_reason.php");
$objFollowupReason = new FollowupReason();
$name            = "";
...
if (isset($_REQUEST['submit'])) 
{
    ...
    if (isset($_REQUEST['mode']) && $_REQUEST['mode'] == "add")
    {
        ...
        $reply = $objFollowupReason->AddFollowupReason($detail);
        if ($reply['ack'] == 1)
        {
    ...
    else if (isset($_REQUEST['mode']) && $_REQUEST['mode'] == "edit")
    {
        ...
        $reply = $objFollowupReason->UpdateFollowupReason($detail);
        if ($reply['ack'] == 1)
        {
            $db->addSuccessMessage($reply['ack_msg']);
...
if (isset($_REQUEST['id']) && $_REQUEST['id'] > 0 && $_REQUEST['mode'] == "edit") 
{
    ...
    $reply = $objFollowupReason->GetFollowupReason($detail);
    if ($reply['ack'] == 1) 
    {
        $result = $reply['result'];
        extract($result);
...
if (isset($_REQUEST['id']) && $_REQUEST['id'] > 0 && $_REQUEST['mode'] == "delete") 
{
    ...
    $detail['id'] = $_REQUEST['id'];
    $reply = $objFollowupReason->DeleteFollowupReason($detail);
    if ($reply['ack'] == 1) 
    {
        $db->addSuccessMessage($reply['ack_msg']);
        $db->rp_location($ctable . "_manage.php?msg=inserted");
    ...
    <script type="text/javascript">
        ...
        function check_form() {
            ...
            if ($("#name").val() == "" || $("#name").val().split(" ").join("") == "") {
                vd = aj.error('name', "Please Enter Followup Reason", "add_error");
                isValid = false;
            }
            if (isValid) {
                return true;
