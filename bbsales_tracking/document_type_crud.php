...
include("connect.php");
require_once("../include/class.document_list_type_add.php");
$objDesignation = new Document();
$name            = "";
$code            = "";
if (isset($_REQUEST['submit'])) {
    ...
    if (isset($_REQUEST['mode']) && $_REQUEST['mode'] == "add") {
        ...
        if ($reply['ack'] == 1) {
            ...
            $db->rp_location("document_list_type_add_manage.php?msg=inserted");
        } else {
        ...
        if ($reply['ack'] == 1) {
            $db->addSuccessMessage($reply['ack_msg']);
            $db->rp_location("document_list_type_add_manage.php?msg=updated");
        } else {
            $db->addErrorMessage($reply['ack_msg']);
        }
    }
    ...
    <script type="text/javascript">
        ...
        function check_form() {
            ...
            if ($("#name").val() == "" || $("#name").val().split(" ").join("") == "") {
                vd = aj.error('name', "Please Enter Document Type Name.", "add_error");
                isValid = false;
            }
            if (isValid) {
                return true;
