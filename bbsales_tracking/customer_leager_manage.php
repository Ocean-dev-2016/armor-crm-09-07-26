<?php
$page_id=587;$page_slug='customer_leager';
$ctable 	= "customer_leager";
$ctable1 	= "Customer Leager";
$main_page 	= $ctable;
$page 		= "manage_".$ctable;
$page_title = $ctable1;
...
<html lang="en"> 
	...
	<body class="page-md">
		...
		<script type="text/javascript">
			var status="";
			var searchName="";
			var data_url = "customer_leager_get_ajax.php";
			...
			function displayRecords(numRecords) { 
				...
				// alert(account_id);
				$("#results" ).html("");
				$("#results" ).load( data_url+"?show=" + numRecords + "&account_id=" + account_id + "&FromDate=" + FromDate + "&ToDate=" + ToDate,function(){
					loadDataTable();
				...
				$("#results").on( "click", ".paging_simple_numbers a", function (e){
					...
					$("#results").load(data_url+"?show=" + numRecords + "&account_id=" + account_id + "&FromDate=" + FromDate + "&ToDate=" + ToDate,{"page":page}, function(){ //get content from PHP page
			...
			function getCustomerList(ctype) {
				...
				$.ajax({
					type: "post",
					url: "ajax_get_customer_plain_list.php",
			...
			function printleager()
			{
		     	...
		     	var myWindow = window.open('print_customer_leager_ajax.php?account_id='+account_id+'&ToDate='+ToDate+'&FromDate='+FromDate,'','width=700,height=800');
		     	myWindow.print();
	        }
		</script>
	</body>
