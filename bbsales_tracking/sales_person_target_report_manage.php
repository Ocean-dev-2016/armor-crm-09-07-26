...
var expense_status="";
/* var filter_month = '';*/
var data_url = "sales_person_target_report_get_ajax.php";
...
function displayRecords(numRecords) {
	...
	$("#results" ).html("");
	$("#results" ).load( data_url+"?show=" + numRecords + "&sales_executive_id=" + sales_executive_id  + "&df=" + df1 + "&searchName=" + searchName +"&filter_month="+filter_month,function(){
		loadDataTable();
	...
	$("#results").on( "click", ".paging_simple_numbers a", function (e){
		...
		$("#results").load(data_url+"?show=" + numRecords + "&sales_executive_id=" + sales_executive_id  + "&df=" + df1  + "&searchName=" + searchName +"&filter_month="+filter_month ,{"page":page}, function(){ //get content from PHP page
	...
	// 	$(".loading-div").show(); //show loading element
	// 	var page = $(this).attr("data-page"); //get page number from link
	// 	$("#results").load(data_url+"?show=" + numRecords + "&searchName="  + "&sales_executive_id=" + sales_executive_id + "&c_id=" + c_id + "&df=" + df1  + "&expense_status=" + expense_status,{"page":page}, function(){ //get content from PHP page
	// 		$(".loading-div").hide(); //once done, hide loading element
	// 		loadDataTable();
	// 	});
...
<script type="text/javascript">
	...
	function genReportexcel(cid){
		...
		$.ajax({
			type: "POST",
			url: "sales_person_target_genReport.php",
			data: '&rc='+rc,
			beforeSend: function() {
				$(".transCover").fadeIn(800);
				$("#loading").modal('show');
...
<script type="text/javascript">
  ...
  searchName     = encodeURIComponent(searchName.trim());
		...
		sales_executive_id = $('#sales_executive_id').val();
		var filter_month=$("#filter_month").val();
     	var myWindow = window.open('print_sales_person_target_report.php?searchName='+searchName+ "&sales_executive_id=" + sales_executive_id + "&filter_month="+filter_month,'','width=700,height=800');
     	myWindow.print();
  //    	setTimeout(function () 
		// {
		// 	myWindow.print();
