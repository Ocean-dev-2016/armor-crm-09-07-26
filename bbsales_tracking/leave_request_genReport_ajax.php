...
{
  $Where .= " leave_category='".$_REQUEST["leave_category"]."' AND ";
}
...
$ctable1_r = $db->rp_getData("leave_request","id,end_date,end_time,start_date,start_time,sales_executive_name,leave_type,created_date,status,cancel_reject_reason,entry_flag,update_entry_flag,leave_category",$Where,"id DESC",0);
...
$column10  = 'K';
  ...
  $objPHPExcel->getActiveSheet()->setCellValue($column1.$rowCount, "Sales Officer Name");
  $objPHPExcel->getActiveSheet()->setCellValue($column2.$rowCount, "Leave Type");
  $objPHPExcel->getActiveSheet()->setCellValue($column3.$rowCount, "Leave Request Created Date");
  ...
  $objPHPExcel->getActiveSheet()->setCellValue($column6.$rowCount, "Leave Category");
  $objPHPExcel->getActiveSheet()->setCellValue($column7.$rowCount, "Total Days Of Leave");
  $objPHPExcel->getActiveSheet()->setCellValue($column8.$rowCount, "Cancel/Reject Reason");
  $objPHPExcel->getActiveSheet()->setCellValue($column9.$rowCount, "Status");
  $objPHPExcel->getActiveSheet()->setCellValue($column10.$rowCount, "Entry Type");
...
$leave_category = array('1' =>"First Half",'2' =>"Second Half",'3' =>"Full Day");
...
{ 
  ...
  for($j=0; $j<10;$j++)  
  {
    ...
    {
      $value = $leave_category[$row['leave_category']];
    }
    else if($j==7)
    {
      $value = $elapsed;
