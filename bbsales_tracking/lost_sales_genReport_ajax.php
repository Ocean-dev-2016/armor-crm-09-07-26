...
$file_name  = "Lost_Sales_Report"."_".date("d-m-Y").".xlsx";
...
$column2  = 'C';
  ...
  $objPHPExcel->getActiveSheet()->setCellValue($column2.$rowCount, "Lost Quotation Count");
...
{  
  ...
  for($j=0; $j<3;$j++)  
  {
    ...
    {
      $value = $db->rp_getTotalRecord("quotation_detail","isDelete=0 AND sales_id='".$row['id']."' AND status=5 ");
    }
    else
    {
      $value = $row[$j];   
