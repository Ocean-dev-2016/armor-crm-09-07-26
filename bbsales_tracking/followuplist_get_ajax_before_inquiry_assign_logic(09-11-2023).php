...
if($_SESSION[SITE_SESS.'_ADMIN_TYPE']!=0)
{
    ...
    {
        $ctable_where .= " user_id='" . $_SESSION[SITE_SESS.'REFERANCE_ID'] . "' AND ";
        $sales_executive=$_REQUEST['sales_executive'];
    }
    else
    {
        if($rights['chain_vise_flag'] == 1)
        {
            ...
            if(!empty($SALEID1))
            {
                $SALEID1=implode(",", $SALEID1);
                    ...
                    $ctable_where .= "   user_id IN (".$SALEID1.','.$_SESSION[SITE_SESS.'REFERANCE_ID'].") AND ";  
                
                
            }
            else
            {
                    $ctable_where .= "   user_id IN (".$_SESSION[SITE_SESS.'REFERANCE_ID'].") AND ";       
            }
        }
        // else
        // {
...
<table id="datatable_1" class="table table-striped table-bordered table-hover table">
        ...
        {
            ?>
                <tr><td class="text-center" colspan="14"><h3><strong><?= FILTER_INFO ?></strong></h3></td></tr>
            <?php
        }
        ?>
        </tbody>
    ...
    $(".datetimerange-picker-input-1").daterangepicker({"format":"dd-mm-yy ",autoUpdateInput: false,timePicker:false,ranges: {
           'Today': [moment(), moment()],
