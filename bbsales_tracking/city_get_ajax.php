...
if(isset($_REQUEST['searchName']) && $_REQUEST['searchName']!=""){
    ...
    $data_search = $db->rp_getData("city","class_id","name = '".$_REQUEST['searchName']."' AND isDelete=0","",0);
    if($data_search)
    {
...
$ctable_r = $db->rp_getData("class","*",$ctable_where,"id DESC limit $page_position, $item_per_page",0);
}
?>
<style>
	.table-scrollable {
	...
	<table id="datatable_sales_executive_type" class="table table-striped table-bordered table-hover">
        <thead class="fix-th">
            ...
            <tr>
                ...
                <th class="fix-th1">State Name</th>
                <th class="fix-th1">City Name</th>
			</tr>
        </thead>
        <tbody>
        <?php
        if ($isFillter) 
        {
	        ...
	        ?>
	            <tr>
					...
					$country_isActive=$db->rp_getValue("country","isActive","id='".$ctable_d['country_id']."' AND isDelete=0");
					...
					<td>
					<?php
					$area=$db->rp_getData("city","name","state_id='".$ctable_d['id']."' AND isDelete=0");
	    ...
	    {
	    	?>
	    		<tr><td colspan="5" class="text-center"><h4><strong>Use filter for the show data of State and city.</strong></h4></td></tr>
	    	<?php
	    }
        ?>
        </tbody>
