...
include("connect.php");
require_once("../include/working.shift.class.php");
$objShift= new Shift();
...
if(isset($_REQUEST['submit'])){
	...
	$detail['start_time']	= $db->clean($_REQUEST['start_time']);
	$detail['end_time']		= $db->clean($_REQUEST['end_time']);
	$detail['isDelete']		= 0;
	if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="add"){
		...
		$reply=$objShift->InsertShift($detail);
		if($reply['ack']==1){
	...
	else if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="edit")
	{
		...
		$reply=$objShift->UpdateShift($detail);
		if($reply['ack']==1){
				$db->addSuccessMessage($reply['ack_msg']);
			$db->rp_location($ctable."_manage.php?msg=inserted");
			}
...
if(isset($_REQUEST['id']) && $_REQUEST['id']>0 && $_REQUEST['mode']=="edit"){
	if($rights['update_flag']!=1)
		...
		$reply=$objShift->EditShift($detail);
		if($reply['ack']==1){
			$SuccessMsg = $reply['ack_msg'];
			$result=$reply['result'];
			extract($result);
		}else{
			$ErrorMsg=$reply['ack_msg'];
		}
...
if(isset($_REQUEST['id']) && $_REQUEST['id']>0 && $_REQUEST['mode']=="delete"){
	...
	$detail['id']=$_REQUEST['id'];
	$reply=$objShift->DeleteShift($detail);
		if($reply['ack']==1){
				$db->addSuccessMessage($reply['ack_msg']);
			$db->rp_location($ctable."_manage.php?msg=inserted");
			}
						...
						<div class="portlet-body form">
							...
							<div class="row">
								...
								<div class="form-group">
									<label>Shift Name <code>*</code></label>
...
$('#start_time').datetimepicker({ format: 'H:i', datepicker: false, autoclose: true });	
$('#end_time').datetimepicker({  format: 'H:i', datepicker: false, autoclose: true });
...
function check_form(){
	...
	if($("#name").val()=="" || $("#name").val().split(" ").join("")==""){		
		vd=aj.error('name',"Please Enter Shift Name.","add_error");
		isValid=false;
	}
	if($("#start_time").val()=="" || $("#start_time").val().split(" ").join("")==""){		
		vd=aj.error('start_time',"Please Enter                                                        Start Time.","add_error");
		isValid=false;
	}
	if($("#end_time").val()=="" || $("#end_time").val().split(" ").join("")==""){		
		vd=aj.error('end_time',"Please Enter End Time.","add_error");
		isValid=false;
	}
	if(isValid)
	{
