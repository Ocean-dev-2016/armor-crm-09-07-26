...
include("connect.php");
$ctable 	= "sales_executive_map_area";
$mode=$_REQUEST['mode'];
...
{	
	if($areaids=="" && $city_id != "" )
	{
		...
		foreach ($item_area as $K) {
			...
			$dup_where1 = "class_id = '".$class_id."' AND  area_id = '".$ares_ids1."'  AND city_id IN(".$city_arr.") AND sales_executive_id='".$sales_executive_id."' AND isDelete=0";

			$r1 = $db->rp_dupCheck("sales_executive_map_area",$dup_where1,0);
			if($r1>0){
				$ack=array("ack"=>0,"ack_msg"=>"This Class And Area Are Already Added.");
			}
			else
			{
				$insert = $db->rp_insert("sales_executive_map_area",array($sales_executive_id,$type,$type_slug,$class_id,$ares_ids1,$city_id_r),array("sales_executive_id","executive_type","type_slug","class_id","area_id","city_id"),0);
			}
		}
	}
	else if ($areaids=="" && $city_id == "")
	{
		...
		$getarea = $db->rp_getData("area","*","class_id='".$class_id."' AND isDelete=0","",0);
		while($getarea_d = mysqli_fetch_assoc($getarea))
		{
			$area_ids1[] = $getarea_d['id'];
		}
			...
			for($i=0;$i<sizeof($areaids);$i++)
			{
				...
				$item[]=array("area_id"=>$areaids[$i]);
					...
					foreach ($item as $i) 
					{
						...
						$dup_where = "class_id = '".$class_id."' AND  area_id = '".$ares_ids."' AND city_id='".$city_r."' AND sales_executive_id='".$sales_executive_id."' AND isDelete=0";

						
						

						// if(in_array($city_r,$city_id))
						// {
						
							$r = $db->rp_dupCheck("sales_executive_map_area",$dup_where,0);
							if($r){
								$ack=array("ack"=>0,"ack_msg"=>"This Class And Area Are Already Added.");
							}
							else
							{
								$insert = $db->rp_insert("sales_executive_map_area",array($sales_executive_id,$type,$type_slug,$class_id,$ares_ids,$city_r),array("sales_executive_id","executive_type","type_slug","class_id","area_id","city_id"),0);
							}
						//}
					}
				}
...
{
	$id=$_REQUEST['id'];
	$delete=$db->rp_update("sales_executive_map_area",array("isDelete"=>1),"id='".$id."'");
	if($delete)
	{
