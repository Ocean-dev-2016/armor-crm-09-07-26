...
if(isset($_REQUEST['searchName']) && $_REQUEST['searchName']!=""){
	$ctable_where .= " (
							...
							OR mobile_no1  LIKE '%".$db->clean($_REQUEST['searchName'])."%'
						) AND ";
}
if($_REQUEST['type']=="super_stockist" )
{
 $where = " AND type_of_executive='dealer' AND super_stockist_id='".$uid."'";
}
if($_REQUEST['type']=="dealer" )
{
 $where = " AND type_of_executive='outlets' AND dealer_distributor_id='".$uid."'";
}
...
{
	$executive_id=array();
	$ctable_area = "class_id=".$_REQUEST['class_name']." AND area_id = '".$_REQUEST['area']."' AND isDelete=0";
...
<form action="" name="frm" id="frm" method="post">
	<table id="datatable_1" class="table table-striped table-bordered table-hover">
        ...
        if(mysqli_num_rows($ctable_r)>0){
            ...
            if($ctable_d['type_of_executive']=="super_stockist"){
				$type="Super Stockist";
			}
			else if($ctable_d['type_of_executive']=="dealer"){
        ...
        ?>
            <tr>
                ...
                <td><span class="<?php echo ($ctable_d['isActive']==0)?"text-danger":"text-success"; ?>"><?php echo stripslashes($ctable_d['cname']); ?></span></td>
				<td><?php echo stripslashes($ctable_d['email']); ?></td>
				<td><?php echo stripslashes($ctable_d['phone']); ?></td>
				<td><?php echo $db->rp_getValue("city","name","name='".$ctable_d['city']."'"); ?></td>
				<td><?php echo $db->rp_getValue("state","name","name='".$ctable_d['state']."'"); ?></td>
				<td><?php echo stripslashes($type); ?></td>
				<?php
				if($_REQUEST['type']=="super_stockist" || $_REQUEST['type']=="dealer")
				{
					?>
					<td>
					<div class="btn-group">
					...
					<ul role="menu" class="dropdown-menu">
						<li>
							<a href="#myModal" data-id="<?php echo  stripslashes($ctable_d['id']); ?>" data-type="<?php echo  stripslashes($ctable_d['type_of_executive']); ?>" data-toggle="modal" title="View Super Stockist"><span class="text-success"><i class="fa fa-circle"></i>&nbsp; View Information</span></a>
						</li>
				...
				<?php 
					if($rights['update_flag']==1)
					{
						...
						{?>
						<a class="btn btn-info btn-sm" onClick="window.location.href='executive_crud.php?mode=edit&type=super_stockist&id=<?php echo $ctable_d['id']; ?>'" title="Edit"><i class="fa fa-pencil"></i></a>
						<?php 
						} 
						
						else if($type_of_executive=='dealer')
						{?>
						<a class="btn btn-info btn-sm" onClick="window.location.href='executive_crud.php?mode=edit&type=dealer&id=<?php echo $ctable_d['id']; ?>'" title="Edit"><i class="fa fa-pencil"></i></a>
						<?php 
						} 
						else
						{?>
						<a class="btn btn-info btn-sm" onClick="window.location.href='executive_crud.php?mode=edit&type=outlets&id=<?php echo $ctable_d['id']; ?>'" title="Edit"><i class="fa fa-pencil"></i></a>
						<?php 
						}
					}
				...
				<div class="btn-group">
					...
					<ul role="menu" class="dropdown-menu">
						...
						<li>
							<a href="#myModal" data-type="<?php echo $type_of_executive;?>" data-id="<?php echo  stripslashes($ctable_d['id']); ?>" data-toggle="modal" title="View Super Stockist"><span class="text-success"><i class="fa fa-circle"></i>&nbsp; View Information</span></a>
						</li>
