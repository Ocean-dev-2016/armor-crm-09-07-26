...
{
      //echo $_REQUEST['df'];exit;
      $date_filter_query = urldecode( $_REQUEST['df2'] );

      $date_filter_query_ex=explode(" to ",$date_filter_query);

      $ctable_where .= " AND ( DATE(end_date)>='".date("Y-m-d",strtotime($date_filter_query_ex['0']))."' AND DATE(end_date)<='".date("Y-m-d",strtotime($date_filter_query_ex['1']))."'  ) ";
}
...
$ctable1_r = $db->rp_getData("master_route","*",$ctable_where,"id DESC",0);
...
$objPHPExcel->getActiveSheet()->setCellValue($column1.$rowCount, "Employee Name");
$objPHPExcel->getActiveSheet()->setCellValue($column2.$rowCount, "Start Date");
$objPHPExcel->getActiveSheet()->setCellValue($column3.$rowCount, "End Date");
$objPHPExcel->getActiveSheet()->setCellValue($column4.$rowCount, "State");
$objPHPExcel->getActiveSheet()->setCellValue($column5.$rowCount, "City");
$objPHPExcel->getActiveSheet()->setCellValue($column6.$rowCount, "Route");
...
{  
  ...
  for($j=0; $j<7;$j++)  
  {
    ...
    {
      $value = date('d-m-Y',strtotime($ctable_d['start_date']));
      
    }
    else if($j==3)
    {
      $value = date('d-m-Y',strtotime($ctable_d['end_date']));
    }
    else if($j==4)
    {
      $value = $db->rp_getValue("class","name","isDelete=0 AND id='".$ctable_d['class_id']."'");
    } 
    else if($j==5)
    {
      $value =  $db->rp_getValue("city","name","isDelete=0 AND id='".$ctable_d['main_city_id']."'");
    } 
    else if($j==6)
    {
      $value = $db->rp_getValue("area","name","isDelete=0 AND id='".$ctable_d['area_id']."'");
    } 
