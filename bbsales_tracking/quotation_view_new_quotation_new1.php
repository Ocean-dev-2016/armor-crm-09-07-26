...
<table>
	<tbody class="<?= $cl; ?>">
		...
		<tr>
			...
			<td colspan="6" class="text-center no-border-right no-border-bottom" style="background-color: <?= VIEW_COLOR ?>"><b>Quotation</b></td>
		...
		<tr class="vertical-top">
			...
			<td colspan="6" class="no-border-bottom no-border-top border-r-width border-blue">
      ...
      $ITEMS[]=$item1;
        ...
        else
        {
        	$grand_total_new=$cart_detail_d['grand_total_rounded']+$cart_detail_d['igst_amount'];
				...
				<tr>
					...
					<td colspan="2" class="text-center carrtoon_qty "><?php echo $currency.' '.round($item['original_price'], 2); ?></td>
					...
					<td colspan="3" class="text-center rate"><?php echo $currency.' '.round($max_total, 2); ?></td>
				</tr>
			<?php
			}
			if($count<5)
		...
		<tr class="font-size">
		 ...
		 if($cart_detail_d['cash_discount']!="" && $cart_detail_d['cash_discount']!=0 && $cart_detail_d['cash_discount_amount']!="" && $cart_detail_d['cash_discount_amount']!=0 && $cart_detail_d['additional_discount']!="" && $cart_detail_d['additional_discount']!=0 && $cart_detail_d['additional_discount_amount']!="" && $cart_detail_d['additional_discount_amount']!=0)
			...
			else if($cart_detail_d['cash_discount']!="" && $cart_detail_d['cash_discount']!=0 && $cart_detail_d['cash_discount_amount']!="" && $cart_detail_d['cash_discount_amount']!=0)
			{
				...
				$new_final_price=$subtotal+$gst_amount+$cart_detail_d['transport_charge']+$cart_detail_d['packing_charge'];
				//13-11-2021 by milan
	      ...
	      $f1=  $db->rp_number_format((float)$fraction, 3, '.', '');
			...
			<td colspan="9" class="no-border-bottom" rowspan="<?= $rowspan; ?>" style="vertical-align: top;">	
				...
				<span class="font-13"><b>Terms & Condition : <?= $hello;?></b></span><br>
				<div class="row">

					<div class="col-md-12" style="height: 165px !important;">
		...
		<tr>
			<td colspan="3" class="text-left font-13"><strong>Cash Discount (<?= $cart_detail_d['cash_discount']; ?> %)</strong></td>			
		...
		<tr>
			<td colspan="3" class="text-left font-13"><strong>Additonal Discount (<?= $cart_detail_d['additional_discount']; ?> %)</strong></td>			
		...
		<tr style="background-color: <?= VIEW_COLOR ?>;font-size: 16px;">
			...
			if(round($f1)<= 0.5)
			{
				$grand_total_new1=$grand_total_new-$f1;
			}
			else
			{
				$grand_total_new1=$grand_total_new-$f1+1;
			}
			?>
			<td colspan="4" class="text-right"><strong><?php echo $currency.' '.$db->rp_number_format($grand_total_new1); ?></strong></td>
		</tr>
