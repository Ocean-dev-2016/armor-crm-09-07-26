...
$page_hierarchy=array(array("link"=>"","title"=>"Master"),array("link"=>"customer_type_manage.php","title"=>"Manage ".$ctable1),array("link"=>$ctable1."_crud.php","title"=>"Add/Edit ".$ctable1));
include("connect.php");
require_once("../include/class.customer_type.php");
$objWeight= new CustomerType();
$name			= "";
$code			= "";
if(isset($_REQUEST['submit'])){
	...
	if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="add"){
		...
		$reply=$objWeight->InsertCustomerType($detail);
		if($reply['ack']==1)
		{
	...
	else if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="edit")
	{
		...
		$reply=$objWeight->UpdateCustomerType($detail);
		if($reply['ack']==1){
			$db->addSuccessMessage($reply['ack_msg']);
...
if(isset($_REQUEST['id']) && $_REQUEST['id']>0 && $_REQUEST['mode']=="edit"){
	if($rights['update_flag']!=1)
		...
		$reply=$objWeight->GetEditDataCustomerType($detail);
		if($reply['ack']==1){
...
if(isset($_REQUEST['id']) && $_REQUEST['id']>0 && $_REQUEST['mode']=="delete"){
	if($rights['delete_flag']!=1)
		...
		$detail['id']=$_REQUEST['id'];
		$reply=$objWeight->DeleteCustomerType($detail);
		if($reply['ack']==1){
		$db->addSuccessMessage($reply['ack_msg']);
		$db->rp_location($ctable."_manage.php?msg=inserted");
		}
...
function check_form(){
	...
	if($("#name").val()=="" || $("#name").val().split(" ").join("")==""){
		...
		vd=aj.error('name',"Please Enter Size Name.","add_error");
		isValid=false;
	}
	if(isValid)
	{
