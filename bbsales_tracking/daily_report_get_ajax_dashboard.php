...
<!-- zoom css -->
  ...
  <button type="button" class="btn btn-sm green-haze print" style="margin-left: -15px;background-color: #f0ad4e;color: #fff; " name="print" onClick="genPrint()" id="print" href="" title="Download XL Report"><i class="fa fa-print"></i>Print</button>
</div>
  <div id="print_info">
  ...
  <div class="col-md-12 col-xs-12 col-sm-12 pad-0" style="margin-bottom: 20px;">
     <span style="font-size: 20px;font-weight:600;">Attendance</span>
  </div>
  ...
  <hr/>
  <div class="col-md-12 col-xs-12 col-sm-12 pad-0" style="margin-bottom: 20px;">
     <span style="font-size: 20px;font-weight:600;">Visit (<?= $db->rp_getTotalRecord($ctable1,$ctable_where_visit); ?>)</span>
  </div>
  <table id="datatable_1" class="table table-striped table-bordered table-hover">
        ...
        if(mysqli_num_rows($ctable_r2)>0)
        {
            ...
            while($ctable_d = mysqli_fetch_array($ctable_r2))
            {
              ...
              <tr>
               ...
               <!-- <td>
                  <a class="mapbtnvisit" data-app_address="<?php echo stripslashes($ctable_d['app_address']); ?>" data-lat="<?php echo stripslashes($ctable_d['latitude']); ?>" data-long="<?php echo stripslashes($ctable_d['longitude']); ?>" data-date="<?=date("d M H:i",strtotime($ctable_d['created_date']));?>" data-salesexename="<?=$db->rp_getValue("sales_executive","name","id='".$ctable_d["user_id"]."'",0);?>" data-toggle="modal" data-target="#OpenVisitMap">
                    <img src="<?=SITEURL?>resource/map.png" style="height: 80px;">
                  </a>
                </td> -->
        ...
        else
        {
          ?>
          <tr>
            <td colspan="6" class="text-center">No Visit Found</td>
          </tr>
          <?php
        }
        ?>
  ...
  <!-- total Survey Customer -->
    <hr/>
  <div class="col-md-12 col-xs-12 col-sm-12 pad-0" style="margin-bottom: 20px;">
      <h3><b>Inquiry Customer (<?= $db->rp_getTotalRecord($ctable2,$ctable_where_inquiry); ?>)</b></h3>
  </div>
    <table id="datatable_1" class="table table-striped table-bordered table-hover">
        <thead>
           <tr>
        ...
        if(mysqli_num_rows($ctable_r4)>0)
        {
            ...
            while($ctable_d4 = mysqli_fetch_array($ctable_r4))
            {
              ...
              <tr>
                ...
                <td>
                    <?php
                    if($ctable_d4['refrence_table']=="sales_executive")
                    {
                      echo $db->rp_getValue("executive","cname","id='".$ctable_d4['visitor_id']."'");
                    }
                    else if($ctable_d4['refrence_table']=="no_order_inquiry")
                    {
                      echo $db->rp_getValue("no_order_inquiry","company_name","id='".$ctable_d4['inquiry_id']."'");
                    }
                    ?>
                </td>
                <td>
                    ...
                    if($ctable_d4['refrence_table']=="sales_executive")
                    {
                      echo $db->rp_getValue("executive","phone","id='".$ctable_d4['visitor_id']."'");
                    }
                    else if($ctable_d4['refrence_table']=="no_order_inquiry")
                    {
                      echo $db->rp_getValue("no_order_inquiry","mobile_number","id='".$ctable_d4['inquiry_id']."'");
                    }
                    ?>
                </td>
  ...
  <hr/>
  <!-- <div class="col-md-12 col-xs-12 col-sm-12 pad-0" style="margin-bottom: 20px;">
     <span style="font-size: 20px;font-weight:600;"><?= $_REQUEST['date']?> Google Map Route</span>
  </div>
  <div>    
   ...
   <!-- Current Date Path -->
   <div class="col-md-6 col-xs-12 col-sm-3" style="margin-bottom: 20px; ">  
   <button type="button" class="btn btn-sm green-haze print" style="background-color: #f0ad4e;color: #fff;" name="print" onClick="genPrint()" id="print" href="" title="Download XL Report"><i class="fa fa-print"></i>Print</button>
   <!-- <button type="button" class="btn btn-sm green-haze pdf" style="margin-left: 20px;" name="pdf" onClick="genPdf()" id="pdf" href="" title="Download XL Report"><i class="fa fa-file-pdf-o"></i>PDF</button> -->
</div>
</div>  
<!-- Modal -->
...
<script type="text/javascript">
  function genPrint()
  {
    ...
    html_data = $("#print_info").html();
    a.document.write(html_data); 
    a.document.close(); 
    a.print(); */
    var URL = <?php ADMINSITEURL ?>"daily_report_get_ajax_dashboard_print.php?id="+id+"&date="+date+"&reqheight=128";
