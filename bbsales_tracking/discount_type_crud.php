<?php
$page_id=639;$page_slug='discount_type';
$ctable 	= "discount_type";
$ctable1 	= "Discount Type";
...
include("connect.php");
require_once("../include/discount_type.class.php");
$objUnit= new Discount_type();
$name			= "";
$code			= "";
if(isset($_REQUEST['submit'])){
	...
	if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="add"){
		...
		$reply=$objUnit->InsertDiscounttype($detail);
		if($reply['ack']==1)
		{
	...
	else if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="edit")
	{
		...
		$reply=$objUnit->UpdateDiscounttype($detail);
		if($reply['ack']==1){
			$db->addSuccessMessage($reply['ack_msg']);
...
if(isset($_REQUEST['id']) && $_REQUEST['id']>0 && $_REQUEST['mode']=="edit"){
	if($rights['update_flag']!=1)
		...
		$reply=$objUnit->EditDiscounttype($detail);
		if($reply['ack']==1){
			$result=$reply['result'];
			extract($result);
		}else{
...
if(isset($_REQUEST['id']) && $_REQUEST['id']>0 && $_REQUEST['mode']=="delete"){
	...
	$detail['id']=$_REQUEST['id'];
	$reply=$objUnit->DeleteDiscounttype($detail);
		if($reply['ack']==1){
		$db->addSuccessMessage($reply['ack_msg']);
		$db->rp_location($ctable."_manage.php?msg=inserted");
		}
	...
	<div class="page-head bg-grey">
		<div class="container">
			<div class="page-title">
				<h1><a href="<?php echo $ctable."_manage.php";?>" class="primary"><i class="fa  fa-arrow-circle-o-left" style="font-size: 22px!important;"></i></a> &nbsp;<?php echo $db->pageBar($page_hierarchy); ?></h1>
...
function check_form(){
	...
	if($("#name").val()=="" || $("#name").val().split(" ").join("")==""){		
		vd=aj.error('name',"Please Enter Unit Name.","add_error");
		isValid=false;
	}
