...
$file_name  = "Product_Item_Sample_Report"."_".strtotime("now").".xlsx";
$ctable_where = "";
...
$column4  = 'E';
  ...
  $objPHPExcel->getActiveSheet()->setCellValue($column3.$rowCount, "hsn code");
  $objPHPExcel->getActiveSheet()->setCellValue($column4.$rowCount, "gst");
  //$column++;
...
{  
  ...
  for($j=0; $j<5;$j++)  
  {
    ...
    {
      $value = $db->rp_getValue("product","hsn_code","isDelete=0 AND id='".$row['product_id']."'",0);
    }
    else if($j==4)
    {
      $value = $db->rp_getValue("product","igst","isDelete=0 AND id='".$row['product_id']."'",0);
    }
    else
    {
      $value = $row[$j];   
