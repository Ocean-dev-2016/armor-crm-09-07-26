...
if(isset($_REQUEST['searchName']) && $_REQUEST['searchName']!=""){
	// $ctable_where .= " (notification_title like '%".$_REQUEST['searchName']."%' OR notification_description like '%".$_REQUEST['searchName']."%') AND ";
	$ctable_where .= " (notification_description like '%".$_REQUEST['searchName']."%') AND ";
}
...
{
	$ctable_where .= "isDelete=0 AND referance_type!='expense' AND user_id='".$_SESSION[SITE_SESS.'_ADMIN_SESS_ID']."' AND notification_type!=1 AND notification_type!=9 ";
}
else
{
    $ctable_where .= "isDelete=0 AND referance_type!='expense'";
}
        ...
        if(mysqli_num_rows($ctable_r)>0)
        {
            ...
            while($ctable_d = mysqli_fetch_array($ctable_r))
            {
	            ...
	            <tr>
	                ...
	                <td>
	                	<?php 
	                		$username =  $db->rp_getValue("dealer_distributor_network","name","sales_executive_id='".$ctable_d['user_id']."'",0); 
	                		if($username=="")
                			{
                			 	echo "Admin";
                			}
