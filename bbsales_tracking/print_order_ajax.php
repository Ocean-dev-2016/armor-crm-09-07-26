...
if(isset($_REQUEST['type']) && $_REQUEST['type']!="" && $_REQUEST['type']!=undefined &&  $_REQUEST['type']!='undefined'){
	$ctable_where .= "  sales_id = '".$db->clean($_REQUEST['type'])."' AND";
}
...
else
{
	if($_REQUEST['order_type'])
	{
		$ctable_where .= " customer_type='".$_REQUEST['order_type']."' AND";
	}
...
if(isset($_REQUEST['df']) && $_REQUEST['df']!=""){
	...
	$ctable_where .= "( DATE(order_date)>='".date("Y-m-d",strtotime($date_filter_query_ex['0']))."' AND DATE(order_date)<='".date("Y-m-d",strtotime($date_filter_query_ex['1']))."'  ) AND ";
}
// --for customer Type//
...
<form action="" name="frm" id="print_info" method="post"> -->
	<table id="datatable_1" class="table table-bordered table-striped dataTable" style="overflow: scroll; !important">
        <thead>
			<tr>
        		<th colspan="12" class="center"><h2>Order Report <?= date("d-m-Y h:i a");?> Printed By : <?=$_SESSION[SITE_SESS.'SESS_NAME'];?></h2></th>
        	</tr>
            <tr class="tr">
