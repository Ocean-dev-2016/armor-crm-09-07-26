                           ...
                           <div class="col-md-7 col-xs-7 col-sm-7 pull-right">
                             <div class="form-inline" role="form">
                                    <form class="form-inline pull-right" role="form" onSubmit="return searchByName();">
                                          ...
                                          <input type="text" style="width: 450px!important" placeholder="Search By  Expence Sub Category  :  " class="form-control input-large" name="searchName" id="searchName" value="" />
...
var status="";
var searchName="";
var expence_category=""
var data_url = "expence_subcategory_get_ajax.php";

function searchByName(){
	searchName = $("#searchName").val();
	expence_category = $("#expence_category").val();
...
function getTopCat(expense_category_id){

		expence_category=expense_category_id;
		searchName = $("#searchName").val();
		var numRecords  = $("#numRecords").val();
		
		var page = $(this).attr("data-page"); //get page number from link
		$("#results").load(data_url+"?show=" + numRecords + "&searchName=" + searchName + "&expence_category=" + expence_category,{"page":page}, function(){ //get content from PHP page
...
function displayRecords(numRecords) {
	...
	expence_category = encodeURIComponent(expence_category.trim());
	$("#results" ).html("");
	$("#results" ).load( data_url+"?show=" + numRecords + "&searchName=" + searchName + "&status=" + status + "&expence_category=" + expence_category,function(){
		loadDataTable();
...
function genSalesExecutiveExcel(){
    	var searchName     = $("#searchName").val();
      	...
      	// searchName     	   = encodeURIComponent(searchName.trim());
      	var state          = $("#state").val();
      	var expence_category          = $("#expence_category").val();
      	var city          = $("#city").val();
      	var type          = $("#sales_executive_type").val();
      	$.ajax({
	        method: "POST",
	        url: "expense_subcategory_excel.php",
	        data:{
 ...
 function genSalesExecutivePrint() {
    	...
    	var myWindow = window.open('print_expense_subcategory_ajax.php?searchName='+searchName+ "&type=" + type + "&state=" + state + "&city=" + city + "&expence_category=" + expence_category ,'','width=700,height=800');
     	myWindow.print();
  //    	setTimeout(function () 
		// {
		// 	myWindow.print();
...
function del_conf(id){
	...
	if(r){
		window.location.href='expence_subcategory_crud.php?mode=delete&id='+id;
	}
}
</script>
</body>
