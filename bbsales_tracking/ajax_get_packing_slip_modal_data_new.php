...
include("connect.php");
/*print_r($_REQUEST['itemarray']); exit;*/
...
if($items)
{
	$newitemarray = $_REQUEST['itemarray'];
	/*print_r($newitemarray); exit;*/
	$count = 0;
	$totalProductQty = 0;
	while ($itemData = mysqli_fetch_assoc($items))
	{
		...
		$Remaingqty = array();
		for ($i=$count; $i < count($newitemarray) ; $i++) { 
			$lastaddqty =  $newitemarray[$i]['addqty'];
			$Rqty =  $newitemarray[$i]['remainigqty'];
			if($Rqty!="")
			{
				$itemData['req'] =  $Rqty - $lastaddqty;
			}
			else
			{
				$itemData['req'] =  $itemData['qty'] - $lastaddqty;
			}
			break;
			//echo "<td>$Remaingqty[$i]</td>";
		}
		$count++;
	?>
	<tr>
		...
		<td><?=$itemData['req']?></td>
		<td style="width: 25%">
			<input data-total-packing="<?=$_REQUEST['total_package']?>" data-pro-id="<?=$itemData['pro_id']?>" data-pro-weight="<?=$itemData['weight_id']?>" data-change-poduct-weight-id="<?=$itemData['pro_id'];?>##<?=$itemData['weight_id'];?>" data-remainig_qty="<?=$itemData['req']?>"  type="text" name="pro_qty[<?=$_REQUEST['total_package']?>][]" class="form-control product-qty"  value="0.000">
		</td>
...
<tr style="background: #eee!important">
	<td colspan="4" class="text-right"><b><?=$_REQUEST['packing_type_name']?> Weight</b></td>
...
<tr>
	<td colspan="4" class="text-right"><b>Actual Weight</b></td>
	<td class="text-center">&nbsp;</td>
	<td class="text-right">
