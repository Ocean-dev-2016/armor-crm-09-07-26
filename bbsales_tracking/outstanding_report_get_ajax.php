...
$current_date  = ($_REQUEST['order_date']!="")?date('Y-m-d',strtotime($_REQUEST['order_date'])):$current_date1;

?> 
<div class="table-responsive" id="print_info">  
	<table id="datatable_11" class="table table-bordered dataTable" style="overflow: scroll; !important;border:1px solid #000" cellpadding="0" cellspacing="0">
        <thead>
			<tr>
            ...
            <tr class="tr">				
                ...
                <th style="border-top:1px solid #000;padding: 2px;border-right: 0;border-left: 0;text-align: center;">Due Days</th>
        ...
        <tbody>
        	...
        	if($customer_r){ 
            	while($customer_d = mysqli_fetch_array($customer_r)){ 
            		// print_r($customer_d);
            		$invoice_where = "isDelete=0 AND customer_id='".$customer_d['id']."' AND order_date<='".$current_date."'" ; 
            		$total_cus_invoice = $db->rp_getTotalRecord("orders",$invoice_where,"",0);
            		if($total_cus_invoice>0)
            		{
                        // echo "SDsd=";
            			$invocie_r = $db->rp_getData("orders","*",$invoice_where,"",0);
        	...
        	if($invocie_r)
        	{
        		...
        		while($invocie_d = mysqli_fetch_assoc($invocie_r))
        		{
                    ...
                    $gtotal = $invocie_d['grand_total_rounded'];
            		...
            		// over credit days
            		// echo $creditdays."-";
            		$diff = strtotime($invocie_d['order_date'])- strtotime($current_date); 
                    ...
                    $tot_amt += $tot_cus_amt;
        	?>
        	<tr style="<?= ($over_creditdays>0)?"background-color: #e87f7f;":"" ?>">
        		<td style="border:0;padding: 2px;text-align: left"><?php if($invocie_d['order_date']!=""){ echo date('d/m/Y',strtotime($invocie_d['order_date'])); }else{ echo "";}?></td>
