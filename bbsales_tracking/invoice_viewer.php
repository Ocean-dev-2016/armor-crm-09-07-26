...
$bid 	= $_REQUEST['invoice_id'];
$invoice_status=$db->rp_getValue("invoice_new","status","id='".$_REQUEST['invoice_id']."' AND isDelete=0");
$customer_id = $db->rp_getValue("invoice_new","customer_id","id='".$_REQUEST['invoice_id']."' AND isDelete=0");
		...
		<div class="container">
			...
			<div class="page-toolbar">
				<?php
							...
							if($flag_d['approve_flag']==1 || $_SESSION[SITE_SESS.'_ADMIN_TYPE']==0)
							{
								?>
									<!-- <div class="btn-group btn-theme-panel hide-app-dis">
										<a class="btn btn-success" href="javascript:;" onClick="InvoiceStatus('<?php echo $bid; ?>','1');" title="Print">Approve</a> -->

									<div class="btn-group btn-theme-panel hide-app-dis">
										<a class="btn btn-success" href='#Invocemodal' data-title="Invoice Detail" data-entry_slug="advance_payment_approve" data-status_slug="4" data-id=".$items[$i]['id']." data-toggle='modal'>Approve</a>
									</div> 
					...
					?>
					<?php
					if($flag_d['export_excel_flag']==1 || $_SESSION[SITE_SESS.'_ADMIN_TYPE']==0)
					{
						?>
						<!-- <div class="btn-group btn-theme-panel">
							<a class="btn dropdown-toggle blue-ebonyclay" href="javascript:;" onClick="genExcel('<?php echo $bid; ?>');" title="Download">Download Excel</a>
						</div> -->
						<?php
					}
					?>
					...
					if($invoice_status==1)
					{
						if($flag_d['email_flag']==1 || $_SESSION[SITE_SESS.'_ADMIN_TYPE']==0)
						{
							?>
								...
								<div class="btn-group btn-theme-panel hide-app-dis">
									<a class="btn btn-success" href='#SendMail' data-title="Invoice" data-id="<?= $_REQUEST['invoice_id'] ?>" data-mailid="<?= $customer_mail_id ?>" data-ccmailid="<?= $customer_ccmail_id ?>" data-type="sales_invoice_detail" data-toggle='modal'>Send Mail</a>
								</div>
							<?php
						}
					}
				?>
					...
					<?php
					$waybill_no = $db->rp_getValue("invoice_new","way_bill_no","id='".$_REQUEST['invoice_id']."' AND isDelete=0");
					//if($waybill_no=="" || $_SESSION[SITE_SESS.'_ADMIN_TYPE']=='12')
					if($_SESSION[SITE_SESS.'_ADMIN_TYPE']=='12')
					{
						?>
						<!-- <div class="btn-group btn-theme-panel hide-app-dis">
							<a class="btn btn-success" href='#EwayBillModel' data-title="Add E-Way Bill No" data-entry_slug="advance_payment_approve" data-status_slug="4" data-id=".$items[$i]['id']." data-toggle='modal'>Add E-Way Bill No</a>
						</div> -->
						<?php
					}
					?>
		...
		<div class="container">
			<div class="row">
				...
				<div class="col-md-12" id="report_content">
					<div id="wrapper1">
						<?php 
						include("invoice_view_new.php");
						//include("invoice_view.php");
						//include("order_view_new_view.php");
						?>
					</div>
...
<!-- PaymentApproveModel -->
<div class="modal" id="EwayBillModel" role="dialog" aria-labelledby="myModalLabel1" >
	<div class="modal-dialog" role="document">
  	<div class="modal-content">
					...
					<div class="modal-body">
						<fieldset class="form-group floating-label-form-group">
	  					<label for="email">E-Way Bill No</label>
	  						<input type="text" class="form-control" name="way_bill_no" id="way_bill_no">
						</fieldset>
					</div>
					<div class="modal-footer">
	  				<button type="button" class="btn grey btn-outline-secondary" data-dismiss="modal">Close</button>
						<button type="button" id="save_waybillno" class="btn btn-success">Save </button>
					</div>
			</form>
  	</div>
	</div>
	...
	<div class="modal-dialog" role="document">
  		<div class="modal-content">
  			<form role="form" action="" method="post" id="formLocation" enctype="multipart/form-data">
				...
				<div class="modal-body">
					<?php 
					$cart_detail_r 	= $db->rp_getData("invoice_new","*","id='".$_REQUEST['invoice_id']."'","",0);
					$cart_detail_d1 	= mysqli_fetch_assoc($cart_detail_r);
					?>
					<fieldset class="form-group floating-label-form-group">
	  					<label for="email">Invoice Series</label><code>*</code>
					...
					<fieldset class="form-group floating-label-form-group">
	  					<label for="email">Invoice No</label><code>*</code>
	  					<input type="text" class="form-control" name="invoice_no" id="invoice_no" value="" readonly>
					</fieldset>
					<fieldset class="form-group floating-label-form-group">
					<label for="email">Invoice Date</label><code>*</code>
...
{	
	var myWindow =  window.open('invoice_view_new_print.php?invoice_id='+id+"&p=1",'','width=500,height=800');
	setTimeout(function () 
	{
        myWindow.print();
...
function printReport(bid){
	...
	$(".invoice_print").on("click",function()
	{
		...
		{
			var myWindow =  window.open('invoice_view_new_print.php?invoice_id='+bid+'&format_type='+print_format_type+"&p=1",'','width=500,height=800');
...
<script type="text/javascript">
	...
	$('#EwayBillModel').on('show.bs.modal', function (event) {
	...
	$(function(){
		$("#save_waybillno").on('click',function(){
			AddEwaybillno();
		});
	})

 	function AddEwaybillno()
 	{
 		var invoice_id = '<?php echo $_REQUEST['invoice_id'] ?>';
 		var way_bill_no=$('#way_bill_no').val();
 		var myFormData = new FormData();
 		myFormData.append('way_bill_no',way_bill_no);
		myFormData.append('invoice_id',invoice_id);
		myFormData.append('mode',"ewaybillno");
...
<script type="text/javascript">
	...
	function UpdateInvoceDetail()
 	{
		...
		myFormData.append('mode',"invoce_detail");
	...
	function getBillNo(typeid)
	{
		...
		$.ajax({
			...
			data: 'typeid='+typeid+'&prefix='+prefix+'&invoice_id='+invoice_id+'&ctable=invoice_new',
			success: function(result){
				var result = $.parseJSON(result);
