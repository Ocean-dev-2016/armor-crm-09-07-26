...
<table>
	<tbody>	
		...
		$cft_print_total=0;
		$ITEMS=array();
		$items1 = $db->rp_getData("packing_slip_item","*","isDelete = 0 AND isActive = 1 AND packing_slip_id = '".$_REQUEST['id']."'","main_carton_type_count ASC",0);
		...
		if($items1)
		{
			...
			foreach($ITEMS as $item)
			{
				...
				$dispatch_inner_size = $db->rp_getValue("order_product_item","box_qty","order_id='".$order_id."' AND pro_id='".$item['pro_id']."' AND weight_id='".$item['weight_id']."' AND isDelete=0",0);
				$dispatch_outre_size = $db->rp_getValue("order_product_item","cartoon_qty","order_id='".$order_id."' AND pro_id='".$item['pro_id']."' AND weight_id='".$item['weight_id']."' AND isDelete=0",0);
				...
				$inner_cbm = $db->rp_getValue("product_weight_price","inner_cbm","product_id='".$item['pro_id']."' AND weight_id='".$item['weight_id']."'",0);
				$outer_cbm = $db->rp_getValue("product_weight_price","outer_cbm","product_id='".$item['pro_id']."' AND weight_id='".$item['weight_id']."'",0);
				


                // if($item['item_size_cft']=="")
                // {
				if($item['main_carton_type']==1)
				{

					if($item['item_size_cft'] != "")
					{
					  $proname_new = $proname ." / CODE ". $procode ." / ". $item['item_size_cft'].' ( Inner )';
					}
					else if($size_inner!="" || $inner_cft!="" || $inner_cbm!="" )
					{
						$proname_new = $proname ." / CODE ". $procode ." / SIZE". $size_inner ." / CFT ". $inner_cft." / CBM".$inner_cbm." ".' ( Inner )';
						$cft_print=$inner_cft;
					}
					else
					{
						$proname_new = $proname ." / CODE ". $procode ." / "."SIZE / CFT / CBM".' ( Inner )';
					}
					
				}
				else if($item['main_carton_type']==2)
				{
					if($item['item_size_cft'] != "")
					{
						$proname_new = $proname ." / CODE ". $procode ." / ". $item['item_size_cft'].' ( Master )';
					}
					else if($size_inner!="" || $outer_cft!="" || $outer_cbm!="" )
					{
						$proname_new = $proname ." / CODE ". $procode ." / SIZE". $size_outer ." / CFT ". $outer_cft." / CBM".$outer_cbm." ".' ( Master )';
						$cft_print=$outer_cft;
					}
					else
					{
						$proname_new = $proname ." / CODE ". $procode ." / "."SIZE / CFT / CBM".' ( Master )';
					}
			...
			// }
				...
				// if($item['main_carton_type']==1)
				// {
				// 	$proname_new = $proname ." / CODE ". $procode ." / SIZE". $size_inner ." / CFT ". $inner_cft.' ( Inner )';
				// }
				// else if($item['main_carton_type']==2)
				// {
				// 	$proname_new = $proname ." / CODE ". $procode ." / SIZE". $size_outer ." / CFT ". $outer_cft.' ( Master )';
				// }
				// else
				// {
				// 	$proname_new = $proname ." / CODE ". $procode ." / ". $item['item_size_cft'];
				...
				<tr>
					...
					<td colspan="9"><?=$proname_new?></td>
					<td colspan="1"><?=$item['value_cft'];?>
						<?php $cft_print_total += $item['value_cft'];?>
					</td>
					<td colspan="1" class="text-center"><?=$item['main_carton_type_count']?></td>
					<td colspan="1" class="text-center"><?=$item['pro_qty']?></td>
					<td colspan="1" class="text-center"><?=$unit_name?></td>
					<td colspan="1" class="text-center"><?=round($item['pro_weight'],2);?></td>
					<td colspan="1" class="text-center"><?=$item['main_carton_whole_actual_weight'];?></td>
				</tr>
				<?php

				$TotalParcelNo  = $db->rp_getValue("packing_slip_item","main_carton_type_count","packing_slip_id='".$_REQUEST['id']."' ORDER BY main_carton_type_count DESC",0);
		...
		<tr>
			...
			<td colspan="9" class="text-center" style="font-weight:700">Total</td>
			<td colspan="1" class="text-center" style="font-weight:700"><?= $cft_print_total?></td>
			<td colspan="1" class="text-center" style="font-weight:700"><?= $TotalParcelNo ?></td>
			<td colspan="1" class="text-center" style="font-weight:700"><?=$MAINTOTALQTY?></td>
			<td colspan="1" class="text-center" style="font-weight:700"></td>
			<td colspan="1" class="text-center" style="font-weight:700"><?=round($MAINTOTALWEIGHT,2);?></td>
			<td colspan="1" class="text-center" style="font-weight:700"><?=$MAINTOTALACTUALWEIGHT;?></td>
		</tr>
		<tr>
			<td colspan="6" rowspan="4" class="no-border-right">
				<br><br><br><br>
				<strong style="margin-right: -58px;">Prepared By</strong>
			</td>
