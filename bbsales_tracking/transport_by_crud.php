...
include("connect.php");
require_once("../include/class.transport_by.php");
$objTransportBy = new TransportBy();
$name            = "";
$code            = "";
if (isset($_REQUEST['submit'])) {
    ...
    if (isset($_REQUEST['mode']) && $_REQUEST['mode'] == "add") 
    {
        ...
        $reply = $objTransportBy->ADDTransportBy($detail);
        if ($reply['ack'] == 1) 
        {
    ...
    else if (isset($_REQUEST['mode']) && $_REQUEST['mode'] == "edit") 
    {
        ...
        $reply = $objTransportBy->UpdateTransportBy($detail);
        if ($reply['ack'] == 1) 
        {
            $db->addSuccessMessage($reply['ack_msg']);
...
if (isset($_REQUEST['id']) && $_REQUEST['id'] > 0 && $_REQUEST['mode'] == "edit") 
{
    ...
    $reply = $objTransportBy->GetTransportBy($detail);
    if ($reply['ack'] == 1) 
    {
        $result = $reply['result'];
        extract($result);
...
if (isset($_REQUEST['id']) && $_REQUEST['id'] > 0 && $_REQUEST['mode'] == "delete") 
{
    ...
    $detail['id'] = $_REQUEST['id'];
    $reply = $objTransportBy->DeleteTransportBy($detail);
    if ($reply['ack'] == 1) 
    {
        $db->addSuccessMessage($reply['ack_msg']);
        $db->rp_location($ctable . "_manage.php?msg=inserted");
                                        ...
                                        <div class="row">
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label>Transport By Name <code>*</code></label>
    ...
    <script type="text/javascript">
        ...
        function check_form() {
            ...
            if ($("#name").val() == "" || $("#name").val().split(" ").join("") == "") {
                vd = aj.error('name', "Please Enter Transport By Name.", "add_error");
                isValid = false;
            }
            if (isValid) {
                return true;
