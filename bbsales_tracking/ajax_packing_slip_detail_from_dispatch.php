	...
	<?php
		...
		if($items){
			...
			while ($item=mysqli_fetch_assoc($items)) {
				...
				//$packing_qty = $db->rp_getValue("packing_slip_item","SUM(pro_qty)","packing_slip_id = '".$_REQUEST['packing_id']."' AND pro_id='".$item['pro_id']."' AND weight_id='".$item['weight_id']."'",0);
				...
				$opening_stock = $db->rp_getValue("product_weight_price","opening_stock_qty","isDelete=0 AND product_id='".$item['pro_id']."' AND weight_id='".$item['weight_id']."'",0);

				$add_manual_qty = $db->rp_getValue("inward_stock","SUM(pro_qty)","isDelete=0 AND pro_id='".$item['pro_id']."' AND weight_id='".$item['weight_id']."'",0);

				$dispatch_qty = $db->rp_getValue("packing_slip_item","SUM(pro_qty)","isDelete=0 AND pro_id='".$item['pro_id']."' AND weight_id='".$item['weight_id']."'",0);


				$get_available_stock=$db->get_available_stock($item['pro_id'],$item['weight_id']);
				$current_Stock=$get_available_stock;


				// $current_Stock = $db->rp_getValue("product_weight_price","stock_qty","isDelete=0 AND product_id='".$item['pro_id']."' AND weight_id='".$item['weight_id']."'",0);

				// $current_Stock = ($add_manual_qty + $opening_stock) - ($dispatch_qty) ;
				?>
			<tr>
				<td><?= ++$count; ?></td>
				<td><?= $item['pro_name']; ?></td>
