...
function genSalesExecutiveExcel(){
    	var searchName     = $("#searchName").val();
      	...
      	$.ajax({
	        method: "POST",
	        url: "source_of_inquiry_excel.php",
	        data:{
 ...
 function genSalesExecutivePrint() {
    	...
    	var myWindow = window.open('print_source_of_inquiry_ajax.php?searchName='+searchName+ "&type=" + type + "&state=" + state + "&city=" + city + "&top_category_id=" + top_category_id ,'','width=700,height=800');
     	myWindow.print();
  //    	setTimeout(function () 
		// {
		// 	myWindow.print();
...
function CheckDispalyOrder(id)
{
	...
	$.ajax({
		...
		data: 'display_order='+display_order+"&id="+size_id+"&table=source_of_inquiry",
		success: function(result){
