...
$invoice_id	= $_REQUEST['pro_forma_invoice_id'];
...
if($cart_detail_d['state']=="Gujarat"){
	$gst=$cart_detail_d['cgst_tax_amount']+$cart_detail_d['sgst_tax_amount'];
}else{
	$gst=$cart_detail_d['igst_tax_amount'];
}
?>
...
<table>
	<tbody>
		...
		<tr>
			...
			<td colspan="8" class="text-center no-border-left"><strong>TAX INVOICE</strong></td>
		...
		<tr>
			...
			<td colspan="9" class="color"><strong>Invoice No. :  <?php echo $cart_detail_d['invoice_no']; ?><br/> Date : <?php echo $invoice_date; ?></strong></td>
		</tr>
		<tr>
		...
		<?php
		$items=$db->rp_getData("proforma_invoice_item","*","proforma_invoice_id='".$invoice_id."'");
		if($items){
			$count=0;
			$cgst_tax=0;
			$sgst_tax=0;
			...
			while ($item=mysqli_fetch_assoc($items)) {
				$count++;
				$rate=$item['item_qty']*$item['item_price'];
			...
			<tr>
				...
				<td colspan="2" class="pname"><?php echo $item['item_name']; ?></td>
				<td colspan="2" class="hsn"><?php echo $item['item_code']; ?></td>
				<td colspan="2" class="text-right qty"><?php echo $db->rp_number_format($item['item_qty']); ?></td>
				<td colspan="2" class="text-right rate"><?php echo $db->rp_number_format($item['item_price']); ?></td>
		...
		<tr class="border">
			...
			<td colspan="2" class="text-right no-border-left"><?php echo $db->rp_number_format($cart_detail_d['cgst_tax_amount'],2)."<br/>".$db->rp_number_format($cart_detail_d['sgst_tax_amount'],2); ?></td>
			<?php
			}
			else
			{
			...
			<td colspan="2" class="text-right no-border-left"><?php echo $db->rp_number_format($cart_detail_d['igst_tax_amount'],2); ?></td>
			<?php
			}
			?>
		</tr>
