	...
	<body>
		...
		<br><br><br><br><br><br><br><br><br><br><br>
				...
				<tr class="vertical-top">
					<td colspan="5" class="no-border-bottom no-border-top border-r-width border-blue">
						...
						<span><b>Contact Person :</b> <?=$Distri_data_a['customer_name']?></span><br>
						<span><b>Contact Number :</b> <?=$Distri_data_a['contact_number']?></span><br>
					...
					<td colspan="6" class="no-border-bottom no-border-top">
						...
						<table class="no-border-top no-border-bottom no-border-right no-border-left" style="width: 100% !important;">
							<tr>
								<td class="no-border-right no-border-bottom no-border-left no-border-top"><b>Quotation No :</b> </td>
							...
							<tr>
								<td class="no-border-right no-border-bottom no-border-left"><b>Inquiry No :</b> </td>
								<td class="no-border-right no-border-bottom no-border-left"><?= "INQ/" . $cart_detail_d['inquiry_id']; ?></td>
							</tr>
							<tr>
								<td class="no-border-right no-border-bottom no-border-left"><b>Inquiry Date :</b> </td>
								<td class="no-border-right no-border-bottom no-border-left"><?php if($inquiry_date!="01-01-1970" && $inquiry_date!=""){ echo date('d-m-Y', strtotime($inquiry_date)); }else{echo "";}?></td>
							</tr>
		...
		<table>
			<tbody class="<?= $cl; ?>">
				<tr class="text-center" style="background-color: <?= VIEW_COLOR ?>;">
					...
					<th colspan="4" class="pname model text-center">Description of Goods</th>
					<!-- <th colspan="1" class="text-center">UNIT</th> -->
					<th class="pname1 text-center">HSN/SAC</th>
					<th colspan="1" class="text-center">GST Rate</th>
					<th colspan="1" class="text-center carrtoon_qty">Quantity</th>
					<th colspan="2" class="rate text-center">Price</th>
					<?php //if($max_discount != "0"){ ?>
					<th colspan="1" class="discount_p text-center">Discount</th>
					<?php //} ?>
					<th colspan="1" class="text-center">Net Rate</th>
					<th colspan="1" class="text-center">Taxable Rate</th>
					<!-- <th colspan="1" class="text-center">GST Amount</th> -->
				...
				if($items1)
				{
					...
					foreach($ITEMS as $item)
					{
						...
						$average_value = ($totalprice/$totalproqty);
						...
						<tr>
							...
							<td colspan="1" class="text-right carrtoon_qty"><?php echo $item['pro_qty']." ".$unit_name; ?></td>
							<td colspan="2" class="text-right rate "><?php echo $currency.' '.round($item['original_price'], 2); ?></td>
							<?php //if($max_discount != "0"){ ?>
							...
							?>
							<td colspan="1" class="text-right"><?php echo $currency.' '.round($rate_total, 2); ?></td>
							<td colspan="1" class="text-right"><?php echo $currency.' '.round($max_total, 2); ?></td>
							<!-- <td colspan="1" class="text-right"><?php echo $currency.' '.round($item['igst_amount'], 2); ?></td> -->
							<!-- <td colspan="<?=$colSpan;?>" class="text-center rate"><?php echo $currency.' '.round($max_total, 2); ?></td> -->
							<!-- <td colspan="<?=$colSpan;?>" class="text-right rate"><?php echo $currency.' '.round($item['subtotal'], 2); ?></td> -->
						</tr>
						<?php
					}
				...
				<tr>
					...
					<td colspan="1" class="box_qty"><b>Total</b></td>
					<td colspan="1" class=""><b><?php echo $totalproqty; ?></b></td>
					<td colspan="2" class="rate"></td>
					<?php //if($max_discount != "0"){ ?>
					<td colspan="1" class="discount_p text-center"><b>Average</b></td>
					<?php //} ?>
					<!-- <td colspan="1" class="rate"></td> -->
					<td colspan="1" class="rate"><b><?php echo round($average_value,3) ?></td>
					<!-- <td colspan="1" class="rate"><?php echo $currency.$totalgstamount; ?></td>
					<td colspan="<?=$colSpan1;?>" class="rate"></td> -->
				</tr>
			</tbody>
		</table>
		<table>
			<tbody class="<?= $cl; ?>">
				<tr class="font-size">
					...
					<td colspan="4" class="text-right font-13"><strong><?php echo $currency.' '.$db->rp_number_format(round($totalprice,2)); ?></strong></td>
				...
				<tr>
					<td colspan="4" class="text-left font-13"><strong>Additonal Discount</strong></td>
				...
				<tr>
					...
					<td colspan="4" class="text-right font-13"><strong><?php echo $currency.' '.$db->rp_number_format(round($total_tax_amt,2)); ?></strong></td>
				</tr>
				<?php
				if($cart_detail_d['igst_amount']!="0")
				{
					...
					if($Distri_data_a['type_of_executive']==7)
					{
						if (strtolower(CLIENT_STATE) == strtolower($Distri_data_a['state']))
						{
							...
							<tr>
								...
								<td colspan="4" class="text-right "><strong><?= ($currency. $db->rp_number_format(($finalgst)/2))?></strong> </td>
							</tr>
							<tr>
						...
						else
						{
							...
							<tr>
								...
								<td colspan="4" class="text-right "><strong><?= ($currency. $db->rp_number_format($finalgst))?></strong></td>
							</tr>
							<tr>
				...
				?>
					<tr>
						...
						<td colspan="4" class="text-right"><strong><?=round($tcs_amount,2) ?></strong></td>
					</tr>
					<?php
				}
				else
				...
				<tr style="background-color: <?= VIEW_COLOR ?>;font-size: 16px;">
					...
					<td colspan="4" class="text-right"><strong>
						...
						{
							$tcs_amount = (($total_tax_amt+$finalgst)*(0.1/100));
							echo $currency.' '.$db->rp_number_format(round($total_tax_amt+$finalgst+$tcs_amount,0));
						}
						else
						{
							echo $currency.' '.$db->rp_number_format(round($total_tax_amt+$finalgst,0));
						}
						?>
						</strong>
					</td>
		...
		<table>
			...
			<?php
					...
					//$Query = "SELECT `invoice_new_product_item`.`id`, `invoice_new_product_item`.`pro_id`, `invoice_new_product_item`.`pro_id`, ( SELECT hsn_code FROM `product` WHERE id = `invoice_new_product_item`.`pro_id` ) AS id FROM `invoice_new_product_item` WHERE isDelete = 0  AND invoice_id='".$invoice_id."' AND dispatch_item_type=1 GROUP By id";
					//echo $Query;exit;
					//$items1 = $db->rp_getQuery($Query);
					...
					while ($item=mysqli_fetch_assoc($items1)) 
					{
						...
						if($items1)
						{
							...
							$GST=0;
								...
								// $cash_discount = $db->rp_getValue("orders","cash_discount","isDelete=0 AND id='".$OrderD['id']."' ",0);

								// $additional_discount1 = $db->rp_getValue("orders","additional_discount","isDelete=0 AND id='".$OrderD['id']."' ",0); 

								// $cash_amount=($total_pro_taxable*$cash_discount)/100;
								...
								// 	$SubPrice1=$add_discount_amount2;
								// }
								// else
								// {
								...
								// 	$add_discount_amount2=($SubPrice*$additional_discount1)/100;
								// 	$SubPrice1=$add_discount_amount2;
								// 	$SubPrice3 = $SubPrice1-$add_discount_amount2;
								// 	// echo $add_discount_amount2; exit;
								// 	// $add_discount_amount2=($SubPrice*$additional_discount1)/100;
								// }
								...
								<tr>
									...
									<td colspan="2" class="text-right b_qty "><?php echo $GST."%"; ?></td>
									<td colspan="2" class="text-right b_qty "><?php echo CURR.' '.number_format($gst_per_amount,2);  ?></td>
									<td colspan="1" class="text-right b_qty "><?php echo $CGST."%"; ?></td>
									<td colspan="2" class="text-right b_qty "><?php echo CURR.' '.number_format($cgst_per_amount,2);  ?></td>
									<td colspan="2" class="text-right b_qty "><?php echo $SGST."%"; ?></td>
									<td colspan="3" class="text-right b_qty "><?php echo CURR.' '.number_format($sgst_per_amount,2);  ?></td>
								</tr>
								<?php
				...
				<tr >
					...
					<td colspan="2" class="text-right"><b><?php echo  CURR.' '.number_format($Total_gst_per_amount,2); ?></b></td>
					<td colspan="1" class="text-right"><b><?php ?></b></td>
					<td colspan="2" class="text-right"><b><?php echo  CURR.' '.number_format($Total_cgst_per_amount,2); ?></b></td>
					<td colspan="2" class="text-right"><b><?php ?></b></td>
					<td colspan="3" class="text-right"><b><?php echo  CURR.' '.number_format($Total_sgst_per_amount,2); ?></b></td>
				</tr>
		</table>
		<!-- hsn summary -->
		<table>
