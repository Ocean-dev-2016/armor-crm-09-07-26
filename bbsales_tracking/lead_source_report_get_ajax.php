	...
	<table id="" class="table table-striped table-bordered table-hover">
        ...
        ?>
            <tr>
                ...
                <td><?= $ctable_d['name'] ?></td>
                <td><?= $db->rp_getTotalRecord("no_order_inquiry","isDelete=0 AND inquiry_lead_flag=1 AND source_of_inquiry='".$ctable_d['id']."' ") ?></td>
                <?php
             ...
             //    while($lead_d=mysqli_fetch_assoc($lead_r))
             //    {
             //    	$quotation=$db->rp_getValue("quotation_detail","id","isDelete=0 AND inquiry_id='".$lead_d['id']."' AND (status = 1 OR status = 2 Or status = 4) ");
             //    	$order+=$db->rp_getValue("orders","SUM(grand_total)","isDelete=0 AND (status = 1 OR status = 2 Or status = 4) AND quotation_id='".$quotation."' ",0);
            	// }
