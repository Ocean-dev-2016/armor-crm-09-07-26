...
$page_hierarchy=array(array("link"=>"","title"=>"Master"),array("link"=>"leave_type_manage.php","title"=>"Manage ".$ctable1),array("link"=>$ctable1."_crud.php","title"=>"Add/Edit ".$ctable1));
include("connect.php");
require_once("../include/class.leave_type.php");
$objLeave= new LeaveType();
$name			= "";
$code			= "";
if(isset($_REQUEST['submit'])){
	...
	if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="add"){
		...
		$reply=$objLeave->InsertLeaveType($detail);
		if($reply['ack']==1)
		{
	...
	else if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="edit")
	{
		...
		$reply=$objLeave->UpdateLeaveType($detail);
		if($reply['ack']==1){
			$db->addSuccessMessage($reply['ack_msg']);
...
if(isset($_REQUEST['id']) && $_REQUEST['id']>0 && $_REQUEST['mode']=="delete"){
	if($rights['delete_flag']!=1)
		...
		$detail['id']=$_REQUEST['id'];
		$reply=$objLeave->DeleteLeaveType($detail);
		if($reply['ack']==1){
		$db->addSuccessMessage($reply['ack_msg']);
		$db->rp_location($ctable."_manage.php?msg=inserted");
		}
...
function check_form(){
	...
	if($("#name").val()=="" || $("#name").val().split(" ").join("")==""){
		...
		vd=aj.error('name',"Please Enter Leave Type.","add_error");
		isValid=false;
	}
	if(isValid)
	{
