...
// $ctable_where_prospect .= " AND DATE(inquiry_date) >= '".date("Y-m-d",strtotime($_REQUEST['date']))."' ";
 $ctable_where_prospect .= " AND DATE(inquiry_date) <= '".date("Y-m-d",strtotime($_REQUEST['date']))."' AND inquiry_lead_flag = '-1'";
...
$ctable_where_inquiry .= " AND DATE(inquiry_date) >= '".date("Y-m-d",strtotime($_REQUEST['date']))."' AND inquiry_lead_flag = '0' ";
...
$ctable_where_lead .= " AND DATE(inquiry_date) <= '".date("Y-m-d",strtotime($_REQUEST['date']))."' AND inquiry_lead_flag = '1'";
...
$ctable_where_packingslip .= " AND DATE(packing_slip_date) >= '".date("Y-m-d",strtotime($_REQUEST['date']))."'";
// $ctable_where_packingslip .= " AND DATE(packing_slip_date) <= '".date("Y-m-d",strtotime($_REQUEST['date1']))."'";
...
$ctable_where_invoice .= " AND DATE(invoice_date) >= '".date("Y-m-d",strtotime($_REQUEST['date']))."'";
// $ctable_where_invoice .= " AND DATE(invoice_date) <= '".date("Y-m-d",strtotime($_REQUEST['date1']))."'";
...
$ctable_where_customer_receipt .= " AND DATE(payment_date) >= '".date("Y-m-d",strtotime($_REQUEST['date']))."'";
// $ctable_where_customer_receipt .= " AND DATE(payment_date) <= '".date("Y-m-d",strtotime($_REQUEST['date1']))."'";
...
$ctable_where_leaverequest .= " AND DATE(start_date) >= '".date("Y-m-d",strtotime($_REQUEST['date']))."'";
//$ctable_where_leaverequest .= " AND DATE(start_date) <= '".date("Y-m-d",strtotime($_REQUEST['date1']))."'";
...
$ctable_where_newcustomer .= " AND DATE(created_date) >= '".date("Y-m-d",strtotime($_REQUEST['date']))."'";
// $ctable_where_newcustomer .= " AND DATE(created_date) <= '".date("Y-m-d",strtotime($_REQUEST['date1']))."'";
...
$ctable_where_Inquiry_followup .= " AND DATE(followup_date) >= '".date("Y-m-d",strtotime($_REQUEST['date']))."'";
//$ctable_where_Inquiry_followup .= " AND DATE(followup_date) <= '".date("Y-m-d",strtotime($_REQUEST['date1']))."'";
...
$ctable_where_lead_followup .= " AND DATE(followup_date) >= '".date("Y-m-d",strtotime($_REQUEST['date']))."'";
// $ctable_where_lead_followup .= " AND DATE(followup_date) <= '".date("Y-m-d",strtotime($_REQUEST['date1']))."'";
...
$ctable_where_Prospects_followup .= " AND DATE(followup_date) >= '".date("Y-m-d",strtotime($_REQUEST['date']))."'";
// $ctable_where_Prospects_followup .= " AND DATE(followup_date) <= '".date("Y-m-d",strtotime($_REQUEST['date1']))."'";
...
$ctable_where_Q_followup .= " AND DATE(followup_date) >= '".date("Y-m-d",strtotime($_REQUEST['date']))."'";
// $ctable_where_Q_followup .= " AND DATE(followup_date) <= '".date("Y-m-d",strtotime($_REQUEST['date1']))."'";
// $ctable_where_Inquiry_followup .= " OR DATE(created_date) = '".date("Y-m-d",strtotime($_REQUEST['date']))."')";
...
$in_time = $db->rp_getData("attendance","date_time","sales_id='".$_REQUEST['sales_id']."' AND DATE(date_time)='".date('Y-m-d',strtotime($date))."' AND inout_status='In'","",0);
  $in = array();
  ...
  $out_time = $db->rp_getData("attendance","date_time","sales_id='".$_REQUEST['sales_id']."' AND DATE(date_time)='".date('Y-m-d',strtotime($date))."' AND inout_status='Out'","",0);
  $out = array();
...
<div class="row" id="report_content1" style="margin:0;">
  <div class="col-md-12 col-xs-12 col-sm-12 pad-0" style="margin: 0">
    ...
    <table id="datatable_1" class="table table-striped table-bordered table-hover">
          ...
          if(mysqli_num_rows($ctable_r3)>0)
          {
              ...
              while($ctable_d3 = mysqli_fetch_array($ctable_r3))
              {
                ...
                $is_followup = $db->rp_getValue("followup","id","isDelete=0 AND reference_table = 'no_order_inquiry' AND reference_id = '".$ctable_d3['id']."' ")
                ?>
                <tr <?php echo ($is_followup) ? 'style="background-color:#208320b8;"' : ''; ?> >
                  <td><?php echo ++$count; ?></td>                
        ...
        if(mysqli_num_rows($ctable_r9)>0)
        {
            ...
            while($ctable_dF = mysqli_fetch_array($ctable_r9))
            {
              ...
              <tr>
                ...
                <td><?php echo  $ctable_dF['description'];?></td>              
                <td><?php echo $msg[$ctable_dF['through']];?></td>              
    ...
    <div class="col-md-12 col-xs-12 col-sm-12 pad-0" style="margin-bottom: 20px;">
      <h3><b>Quotation Followup (<?= $db->rp_getTotalRecord($ctable6,$ctable_where_Q_followup); ?>)</b></h3>
    </div>
     <table id="datatable_1" class="table table-striped table-bordered table-hover">
        <thead>
           <tr>
        ...
        if(mysqli_num_rows($ctable_r12)>0)
        {
            ...
            while($ctable_dQ = mysqli_fetch_array($ctable_r12))
            {
              ...
              <tr>
                <td><?php echo ++$count; ?></td>    
                <td><?php echo $db->rp_getValue("quotation_detail","quotation_no","id='".$ctable_dQ['reference_id']."'",0); ?></td>                            
                <td>
                    <?php
                    if($ctable_dQ['reference_table']=="quotation_detail")
                    {
                      $customer_id=$db->rp_getValue("quotation_detail","customer_id","id='".$ctable_dQ['reference_id']."'");
                      echo $db->rp_getValue("executive","cname","id='".$customer_id."'",0);
                    }
                    else if($ctable_dQ['reference_table']=="quotation_detail")
                    {
                      echo $db->rp_getValue("no_order_inquiry","company_name","id='".$ctable_dQ['reference_id']."'",0);
                    }
                    ?>
                </td>
                ...
                <td>
                    ...
                    {
                      echo $db->rp_getValue("no_order_inquiry","mobile_number","id='".$ctable_dQ['reference_id']."'",0);
                    }
                    ?>
                </td>
                <td>
                    <?php if($ctable_dQ['reference_table']=="quotation_detail")
                    {
                      echo $db->rp_getValue("sales_executive","name","id='".$ctable_dQ['user_id']."'");
                    }
                    ?>
                  </td>              
                <td><?php echo $ctable_dQ['description'] ?></td>              
                <td><?php echo $msg[$ctable_dQ['through']]?></td>              
                <?php if($ctable_dQ['followup_date']=="0000-00-00 00:00:00"){?>
                  <td></td>
                <?php 
                } else { ?>
                <td><?php echo date('d-m-Y H:i:s a',strtotime($ctable_dQ['followup_date'])); ?></td>
                <?php } ?>           
                <td>
                    <?php 
                    if($ctable_dQ['reference_table']=="quotation_detail")
                    {
                      echo $ctable_dQ['response'];
                    }
                    ?>
                </td>
              </tr>
  ...
  <!--   <hr/>
   ...
   <!--  <hr/> -->
    <!-- <div class="col-md-12 col-xs-12 col-sm-12 pad-0" style="margin-bottom: 20px;">
      <h3><b>Payment Colloection (<?= $db->rp_getTotalRecord($ctable14,$ctable_where_customer_receipt); ?>) --- Total Amount (<?= CURR .$db->rp_number_format($db->rp_getValue($ctable14,"SUM(paid_amount)",$ctable_where_customer_receipt),2); ?> )</b></h3>
    </div> -->
    <!-- <table id="datatable_1" class="table table-striped table-bordered table-hover">
      <thead>
        <tr>
  ...
  <script type="text/javascript">
    $(".mapbtn").click(function(){
        ...
        {
          $.ajax({
            url: "get_attendance_map_dashboard.php",
            data: {
              lat: lat,
              lng: lng,
              date: date,
    ...
    {    
        ...
        $.ajax({
          url: "get_visit_map_dashboard.php",
          data: {
            lat: lat,
            lng: lng,
            date: date,
