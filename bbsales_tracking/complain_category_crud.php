<?php
$page_id=645;$page_slug='complain_category';
$ctable 	= "complain_category";
$ctable1 	= "Complain Category";
$main_page 	= "product_mgmt";
$page 		= "manage_".$ctable;
$page_title = ucwords($_REQUEST['mode'])." Complain/Request Category";
$page_hierarchy=array(array("link"=>"","title"=>"Master"),array("link"=>"complain_c'ategory_manage.php","title"=>"Manage Complain/Request Category"),array("link"=>$ctable1."_crud.php","title"=>"Add/Edit Complain/Request Category"));
include("connect.php");
require_once("../include/class.complain_category.php");
$objWeight= new ComplainCategory();
$name			= "";
$code			= "";
if(isset($_REQUEST['submit'])){
	...
	if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="add"){
		...
		$reply=$objWeight->InsertComplainCategory($detail);
		if($reply['ack']==1)
		{
	...
	else if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="edit")
	{
		...
		$reply=$objWeight->UpdateComplainCategory($detail);
		if($reply['ack']==1){
			$db->addSuccessMessage($reply['ack_msg']);
...
if(isset($_REQUEST['id']) && $_REQUEST['id']>0 && $_REQUEST['mode']=="edit"){
	if($rights['update_flag']!=1)
		...
		$reply=$objWeight->GetEditDataComplainCategory($detail);
		if($reply['ack']==1){
...
if(isset($_REQUEST['id']) && $_REQUEST['id']>0 && $_REQUEST['mode']=="delete"){
	if($rights['delete_flag']!=1)
		...
		$detail['id']=$_REQUEST['id'];
		$reply=$objWeight->DeleteComplainCategory($detail);
		if($reply['ack']==1){
		$db->addSuccessMessage($reply['ack_msg']);
		$db->rp_location($ctable."_manage.php?msg=inserted");
		}
...
function check_form(){
	...
	if($("#name").val()=="" || $("#name").val().split(" ").join("")==""){
		...
		vd=aj.error('name',"Please Enter Complain/Request Category Name.","add_error");
		isValid=false;
	}
	if(isValid)
	{
