<?php
$page_id=613;$page_slug='warehouse-master';
$ctable 	= "warehouse";
$ctable1 	= "Warehouse Master";
...
include("connect.php");
require_once("../include/class.warehouse_master.php");
...
if(isset($_REQUEST['submit'])){
	...
	if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="add"){
		...
		$reply=$objWarehouse->InsertWarehouse($detail);
		if($reply['ack']==1)
		{
	...
	else if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="edit")
	{
		...
		$reply=$objWarehouse->UpdateWarehouse($detail);
		if($reply['ack']==1){
			$db->addSuccessMessage($reply['ack_msg']);
...
if(isset($_REQUEST['id']) && $_REQUEST['id']>0 && $_REQUEST['mode']=="edit"){
	if($rights['update_flag']!=1)
		...
		$reply=$objWarehouse->GetEditDataWarehouese($detail);
		if($reply['ack']==1){
...
if(isset($_REQUEST['id']) && $_REQUEST['id']>0 && $_REQUEST['mode']=="delete"){
	if($rights['delete_flag']!=1)
		...
		$detail['id']=$_REQUEST['id'];
		$reply=$objWarehouse->DeleteWarehouse($detail);
		if($reply['ack']==1){
		$db->addSuccessMessage($reply['ack_msg']);
		$db->rp_location($ctable."_manage.php?msg=inserted");
		}
						...
						<div class="portlet-body form">
							...
							<div class="row">
								...
								<div class="form-group">
									<label>Warehouse Name <code>*</code></label>
...
function check_form(){
	...
	if($("#name").val()=="" || $("#name").val().split(" ").join("")==""){
		...
		vd=aj.error('name',"Please Enter Warehouse Name.","add_error");
		isValid=false;
	}
	if(isValid)
	{
