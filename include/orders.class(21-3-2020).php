	...
	public function PlaceOrder($detail)
	{
		...
		if($order_id!=0)
		{
			...
			if($order_items_r)
			{
				...
				$dt=date("Y-m-d");
				$isUpdated=$this->db->rp_update("orders",array("total_qty"=>$total_qty,"subtotal"=>$sub_total,"grand_total"=>$grand_total,"cash_discount_amount"=>$cash_discount_amount,"cash_discount"=>$cash_discount,"booking"=>$detail['booking'],"transport"=>$detail['transport'],"igst_amount"=>$gst_amount,"status"=>0,"order_date"=>$dt,"class_id"=>$detail['class_id'],"area_id"=>$detail['area_id'],"dealer_id"=>$detail['dealer_id']),"id='".$order_id."'",0);
				if($isUpdated)
				{
						///////////////////////// notification ////////////////////
					$created_date=date("Y-m-d H:i:s");
					$title_description="Order of <b>Qty.".$total_qty."</b> for date <b>".date('d-m-Y',strtotime($created_date))."</b> added by <b>".$customer_r['name']."</b>";
					$notification=$this->system->setNotification(0,1,"Order Placed By <b>".$customer_r['name']."</b>",6,"Order Message",$title_description,"","",$created_date,$order_id,"orders","normal_user");

					$customer_order_detail=$this->db->rp_getData("orders","*","id='".$order_id."'");
					if($customer_order_detail)
					{
						$customer_order_detail=mysqli_fetch_assoc($customer_order_detail);
						$customer_order_items=$this->db->rp_getData("order_product_item","*","order_id='".$order_id."'");
						if($customer_order_items)
						{
							while($customer_order_item=mysqli_fetch_assoc($customer_order_items))
							{
