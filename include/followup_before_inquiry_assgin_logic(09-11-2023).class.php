...
class Followup extends Functions
{
	...
	{		
		...
		$Values=array($reference_table,$user_id,$visitor_id,$reference_id,$project_manager_id,$description,$through,date("Y-m-d H:i:s",strtotime($followup_date)),0,1,$next_followup_id,$refrence_media_id,$entry_type);

		$Columns=array("reference_table","user_id","visitor_id","reference_id","project_manager_id","description","through","followup_date","isDelete","isActive","next_followup_id","refrence_media_id","entry_type");
	...
	public function GetFollowupContent($visitor_id,$reference_id)
	{
		...
		else if($_REQUEST['sales_id']!="" && $_REQUEST['customer_id']=="")
		{
			...
			$Contents=$this->db->rp_getTotalRecord($this->ctable,"isDelete='0' AND isActive='1' AND (visitor_id='".$visitor_id."' OR reference_id='".$refrence_id."') AND user_id='".$_REQUEST['sales_id']."'",0);
		}
		else if($_REQUEST['sales_id']!="" && $_REQUEST['customer_id']!="")
		{
		...
		{	
			if($_REQUEST['customer_id']!="" && $_REQUEST['reference_id']=="")
			{
				...
				$FollowupContent=$this->db->rp_getData($this->ctable,"*","isDelete='0' AND isActive='1' AND (visitor_id='".$_REQUEST['customer_id']."' OR reference_id='".$_REQUEST['customer_id']."') AND reference_table = 'executive'".$Where,"followup_date DESC",0,$limit);
			}
			else if($_REQUEST['customer_id']=="" && $_REQUEST['reference_id']=="")
			{
				$FollowupContent=$this->db->rp_getData($this->ctable,"*",$followup_type_where."isDelete='0' AND isActive='1' AND user_id='".$_REQUEST['sales_id']."' ".$Where,"followup_date DESC",0,$limit);
			}
			else
			{
	...
	public function GetTodayFollowup($user_id,$limit="")
	{
		...
		{
			$admin_type=$this->db->rp_getValue("sales_executive","type","id='".$user_id."'",0);
			$where1="isDelete='0' AND isActive='1' AND visitor_id IN (".implode(",",$vistors).") OR reference_id IN (".implode(",", $inquiry).")";
			if($admin_type==0)
			{
			...
			{	
				...
				else
				{
					...
					if($_REQUEST['sales_id']!="")
					{
						...
						$FollowupContent=$this->db->rp_getData($this->ctable,"*",$followup_type_where."isDelete='0' AND isActive='1' AND DATE(followup_date)<='".date("Y-m-d")."' AND user_id='".$_REQUEST['sales_id']."' AND response='' ".$where,"followup_date DESC",0,$limit);
					}
					else
					{
