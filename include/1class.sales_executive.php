...
//---------------------Track Sales--------------------------------------------------//	
		...
		if($sales_routes)
		{
			while($route=mysqli_fetch_assoc($sales_routes))
			{
				...
				if($route['sales_executive_id']!="")
				{
					...
					$data_d = $this->rp_getData("salesexecutive_tracking","*",$where1,"id desc",0);
				}
	...
	function trackSalesAll($date="",$id="")
	{
		...
		// $sales_routes=$this->db->rp_getQuery("select * from(select  * from salesexecutive_tracking WHERE ".$where." ORDER BY id DESC ".$limit.") AS x GROUP BY sales_executive_id",0);
		// april month tbale chagne by ravi
		if(date("m",strtotime($date))=="04")
		{
		    $this->ctableTracking="salesexecutive_tracking_april";
		}
		$sales_routes=$this->db->rp_getData($this->ctableTracking,"MAX(id) AS id",$where."  GROUP BY sales_executive_id","id ASC",0,$limit);
		while($route=mysqli_fetch_assoc($sales_routes))
		{
			$trckingids[] = $route['id'];
		}
		if(sizeof($trckingids)>0){
			$trckingids = implode(",", $trckingids);
			$sales_routes=$this->db->rp_getData($this->ctableTracking,"sales_executive_id,latitude,longitude,date,type,app_address","ID IN (".$trckingids.")");
			// $sales_routes=$this->db->rp_getData("salesexecutive_tracking","*","ID IN (".$trckingids.")");
			while($route=mysqli_fetch_assoc($sales_routes))
			{
			    // print_r($route);exit;
				// this code for status (offline/online) purpose only
				...
				if($route['sales_executive_id']!="")
				{
					...
					$where1 .=" AND sales_executive_id=".$route['sales_executive_id']."";
					$where1.=" AND DATE(date)='".date("Y-m-d")."'";
					$data_d = $this->db->rp_getData("salesexecutive_tracking","*",$where1,"id DESC",0);
				}
				$data = mysqli_fetch_assoc($data_d);
				$last_time = $data['date'];
				if($last_time)
		...
		{
			$reply=array("ack"=>0,"ack_msg"=>"No Data Available!!","result"=>"");
		}
		return $reply;
	}
	...
	function find_total_distance_of_sales_executive($sales_id,$date)
	{
		...
		while($tracking_d=mysqli_fetch_assoc($tracking_r)){ 
		  ...
		  if($totData!=$cnt)
		  {
		    ...
		    $next_id=$tracking_d['id'];
		    $second_lat=$this->db->rp_getValue($tableTracking,"latitude",$distance_where." AND id>'".$next_id."'");
		    $second_long=$this->db->rp_getValue($tableTracking,"longitude",$distance_where." AND id>'".$next_id."'"); 
