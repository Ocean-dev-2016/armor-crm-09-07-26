...
{
	public $db;
	public $ctable="top_category_master";
	...
	public function InsertCategory($detail,$file) 
	{
		...
		if($r){
			$reply=array("ack"=>0,"developer_msg"=>"Already Exist Category Name","ack_msg"=>"Duplication! Already Exist Category Name.");
			return $reply;
		}
		else
		{
			...
			if($uid!=0)
			{
				...
				$catId = ",".$uid;
				/* All Customer Category Update IN executive Table */
					$allCustomerCatUpdateSql = "UPDATE executive SET top_category_id = CONCAT(top_category_id, '".$catId."') WHERE isDelete = 0";
					// echo $allCustomerCatUpdateSql;exit;
					$allCustomerCatUpdate = $this->db->query($allCustomerCatUpdateSql);
				/* All Customer Category Update IN executive Table */


				/* All SALES Category Update IN sales_executive Table */
					$catId = ",".$uid;
					$allSalesCatUpdateSql = "UPDATE sales_executive SET top_category_id = CONCAT(top_category_id, '".$catId."') WHERE isDelete = 0";
					// echo $allSalesCatUpdateSql;exit;
					$allSalesCatUpdate = $this->db->query($allSalesCatUpdateSql);
				/* All SALES Category Update IN sales_executive Table */

				/* *** Added By DINESH *** */
				$reply=array("ack"=>1,"developer_msg"=>"Top Category Added.","ack_msg"=>"Success! Top Category Insert Successfully.");
				return $reply;
			}
			else
			{
				$reply=array("ack"=>0,"developer_msg"=>"Database error!!","ack_msg"=>"Failed! Category Insert Failed.");
				return $reply;
			}
		}
	}
	 ...
	 public function UpdateCategory($detail,$file)
	  {
			...
			$r = $this->db->rp_dupCheck($this->ctable,$dup_where);
			if($r){
				$reply=array("ack"=>0,"developer_msg"=>"Already Exist TopCategory Name","ack_msg"=>"Duplication! Already Top Exist Category Name.");
				return $reply;
				...
				if($uid!=0)
				{
					$reply=array("ack"=>1,"developer_msg"=>"Top Category Update Successfull!!.","ack_msg"=>"Success! Top Category Update Successfully.");
					return $reply;
				}
				else
				{
	...
	{		
		...
		$result['customer_unit_id'] = $ctable_d['customer_unit_id'];
		$result['unit_id'] = $ctable_d['unit_id'];
	...
	public function DeleteCategory($detail)
	{
		...
		if($uid!=0)
		{
			$reply=array("ack"=>1,"developer_msg"=>"deleted data.","ack_msg"=>"Success! Delete Top Category Successfully.");
			return $reply;
		}
		else
		{
			$reply=array("ack"=>0,"developer_msg"=>"Database error!!","ack_msg"=>"Failed! Delete Top Category Failed.");
			return $reply;
		}
	}
}
