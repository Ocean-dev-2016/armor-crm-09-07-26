...
class ClassType extends Functions
{
	...
	public function ClassTypeInsert($detail) 
	{
		...
		else
		{
			...
			/*log entry*/
		 	...
		 	$uid = $this->db->rp_insert($this->ctable,$values,$rows,1,$log_description,$flag,$module_name,"","");
			...
			if($uid!=0)
			{
				/*insert to state*/
					...
					$inserted_value = array($name,$country_id,0);
					$insert = $this->db->rp_insert("state",$inserted_value,$inserted_row,0,$log_description,$flag,$module_name,"","");
				/*insert to state*/
				$reply=array("ack"=>1,"developer_msg"=>"Class Added.","ack_msg"=>"Success! Class Added Successfully.");
				return $reply;
			}
			else
			{
	...
	public function ClassTypeUpdate($detail)
	{
			...
			if($r){
				...
				if($uid!=0)
				{
					...
					$reply=array("ack"=>1,"developer_msg"=>"Class Updated Successfully!!.","ack_msg"=>"Success! Class Updated Successfully.");
					return $reply;
				}
				else
				{
					$reply=array("ack"=>0,"developer_msg"=>"Database error!!","ack_msg"=>"Failed! Class Could Not Be Updated .");
					return $reply;
				}
			}	
		}	
	...
	{		
		...
		if($ctable_r)
		{
			...
			$result['country_id']		= htmlentities($ctable_d['country_id']);
	...
	public function ClassTypeDelete($detail)
	{
		...
		"isDelete"	=> "1"
			...
			if($uid!=0)
			{
				$reply=array("ack"=>1,"developer_msg"=>"deleted Class Type.","ack_msg"=>"Success! Class Record Deleted Successfully.");
				return $reply;
			}
			else
			{
				$reply=array("ack"=>0,"developer_msg"=>"Database error!!","ack_msg"=>"Failed! Class Record Could Not Be Deleted.");
				return $reply;
			}
	}
	...
	function getcityDetail_usingClassId($class_id,$sales_id)
	{
		/*$result=array();
		...
		*/
		...
		{
		 	$city_ids=array();
		 	$area_d=$this->db->rp_getData("sales_executive_map_area","city_id","sales_executive_id='".$sales_id."' AND isDelete=0","",0);
		 	if($area_d)
		 	{
			...
			$where="id IN(".$city_ids.") AND isDelete=0 AND state_id='".$class_id."'";
		}
		else
		{
		 	$where="isDelete=0 AND state_id='".$class_id."'";
		}
		$result=$this->db->rp_getData("city","id,name,state_id",$where,"name ASC",0);
	...
	function getAreaDetail_usingClassId($city_id,$sales_id)
	{
		/*$result=array();
		$where="class_id=".$_REQUEST['class_id']."";
		$data    = $this->db->rp_getData('area',"id,name,class_id",$where,"",0,$limit);
		...
		*/
		...
		if($sales_id!="")
		{
			...
			$where="id IN(".$area_ids.") AND isDelete=0 AND city_id='".$city_id."'";
		}
		else
		{
		 	$where="isDelete=0 AND city_id='".$city_id."'";
		}
		$result=$this->db->rp_getData("area","id,name,class_id,isDelete,isActive",$where,"name ASC",0);
        while($detail=mysqli_fetch_assoc($result))
       	{
           $p[]=$detail;
        }
