...
//------------------Delete Order Also Delete product item-----------------	//
	public function DeleteOrder($detail)
	{
		...
		"isDelete"	=> "1"
			...
			$order_id=$this->db->rp_update($this->ctable,$rows,$where);
	...
	public function AddToCart($detail,$products)
	{
		...
		if($customer_d)
		{
			...
			if($check_cart_exist!=0)
			{
				...
				{
					$order_id=$this->db->rp_getValue("orders","id","customer_id='".$detail['cid']."' AND isDelete=0 AND status=-1",0);
				}
			...
			else
			{
				...
				$order_id = $this->db->rp_insert("orders",$values,$rows,0);
				if($order_id!=0)
				{
					if(!empty($products))
					{
	...
	public function PlaceOrder($detail)
	{
		...
		$order_id=$this->db->rp_getValue("orders","id","customer_id='".$detail['cid']."' AND sales_id='".$detail['sales_executive_id']."' AND isDelete=0 AND status=-1".$where,0);
		if($order_id!=0)
		{
			...
			if($order_items_r)
			{
				...
				$dt=date("Y-m-d");
				$isUpdated=$this->db->rp_update("orders",array("total_qty"=>$total_qty,"subtotal"=>$sub_total,"grand_total"=>$grand_total,"cash_discount_amount"=>$cash_discount_amount,"cash_discount"=>$cash_discount,"booking"=>$detail['booking'],"transport"=>$detail['transport'],"igst_amount"=>$gst_amount,"status"=>-1,"order_date"=>$dt,"class_id"=>$detail['class_id'],"area_id"=>$detail['area_id'],"dealer_id"=>$detail['dealer_id']),"id='".$order_id."'",0);
				if($isUpdated)
				{
					///////////////////////// notification ////////////////////
					...
					$result=$objPushNotification->send_CustomerNotification($msg,$refreshTokens,0);
	...
	public function UpdateCartDiscount($detail,$discount)
	{
		if($detail['mode']=="edit")
		{
			$check_cart_exist=$this->db->rp_getTotalRecord("orders","isDelete=0 AND sales_id='".$detail['sales_executive_id']."'",0);
		}
		else
		{
			$check_cart_exist=$this->db->rp_getTotalRecord("orders","isDelete=0 AND sales_id='".$detail['sales_executive_id']."' AND status=-1",0);
		}
		if($check_cart_exist!=0)
		{
			if(!empty($discount))
			{
				...
				{
					$get_item = $this->db->rp_getData("order_product_item","*","isDelete=0 AND order_id='".$detail['cart_id']."'","",0);
					while($get_item_d = mysqli_fetch_assoc($get_item))
					{
						if($d['tcid']==$get_item_d['top_cat_id'] && $d['cid']==$get_item_d['cat_id'])
						{
							...
							$update = $this->db->rp_update("order_product_item",array("discount"=>$d['discount'],"discount_amount"=>$discount_amount,"unitprice"=>$unitprice,"totalprice"=>$totalprice),"order_id='".$detail['cart_id']."' AND id='".$get_item_d['id']."'",0);
						}
					}
				}
	...
	public function UpdateToCart($detail,$products)
	{
		...
		if($customer_d)
		{
			...
			if($check_cart_exist!=0)
			{
				...
				if(!empty($products))
				{
					$this->db->rp_delete("order_product_item","order_id='".$detail['order_id']."'",0);
					$customer_r=mysqli_fetch_assoc($customer_d);
					$price_list_id=$customer_r['price_list_id'];
					foreach ($products as $p)
					{
					...
					$grand_total=$this->db->rp_num($sub_total+$gst_amount);
					$dt=date("Y-m-d");
					$isUpdated=$this->db->rp_update("orders",array("total_qty"=>$total_qty,"subtotal"=>$sub_total,"grand_total"=>$grand_total,"cash_discount_amount"=>$cash_discount_amount,"cash_discount"=>$cash_discount,"igst_amount"=>$gst_amount,"status"=>0,"order_date"=>$dt),"id='".$detail['order_id']."'",0);


					$total_items=$this->db->rp_getTotalRecord("order_product_item","order_id='".$detail['order_id']."' AND isDelete=0");
					if($total_items!=0)
					{
						$reply=array("ack"=>1,"developer_msg"=>"Order Added To Cart Successfully","ack_msg"=>"Order Added To Cart Successfully","order_id"=>$detail['order_id']);
						return $reply;
					}
					else
					{
