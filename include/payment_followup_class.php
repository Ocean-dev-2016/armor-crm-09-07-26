 <?php
	...
	class FollowupPayment extends Functions
	{
		public $db, $log;
		public $ctable = "payment_followup";
		...
		$project_manager_id = $this->db->rp_getValue("executive","project_manager_id","id='".$visitor_id."'",0);*/
			...
			$count = $this->db->rp_getTotalRecord("customer_payment_followup", "reference_id='" . $reference_id . "' AND reference_table='" . $reference_table . "' AND isDelete=0", 0);
			//echo $count; exit;
		...
		{*/
			...
			if ($ContentID) {
				...
				$inquiry_id = $this->db->rp_getValue("payment_followup", "reference_id", "id='" . $followup_id . "' AND isDelete=0");
		...
		public function GetFollowupDetail($followup_id)
		{
			...
			if ($FollowupContent) {
				...
				if ($FollowupContent_d['reference_table'] == "no_order_inquiry") {
					...
					$FollowupContent_d['followup_slug'] = "customer_payment_followup";
				} else if ($FollowupContent_d['reference_table'] == "quotation_detail") {
		...
		public function DeleteFollowup($detail)
		{
			...
			$uid = $this->db->rp_update("payment_followup", $rows, $where, 0);
			if ($uid != 0) {
