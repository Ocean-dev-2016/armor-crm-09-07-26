...
class Unit extends Functions
{
	...
	public function InsertUnit($detail) 
	{
		...
		else
		{
			...
			if($uid!=0)
			{
				$reply=array("ack"=>1,"developer_msg"=>"Unit insert successfully!!.","ack_msg"=>"success! Insert Unit Successfully.");
				return $reply;
			}
			else
			{
				$reply=array("ack"=>0,"developer_msg"=>"Database error!!","ack_msg"=>"Unit insert Failed.");
				return $reply;
			}
		}
	}
		...
		public function UpdateUnit($detail)
	{
			...
			$r = $this->db->rp_dupCheck($this->ctable,$dup_where);
			if($r){
				$reply=array("ack"=>0,"developer_msg"=>"Duplicate Hsn Code","ack_msg"=>"Duplication! Already Exist Unit.");
				return $reply;
				...
				if($uid!=0)
				{
					...
					$reply=array("ack"=>1,"developer_msg"=>"Class Updated Successfully!!.","ack_msg"=>"Success!Unit Updated Successfully.");
					return $reply;
				}
				else
				{
					$reply=array("ack"=>0,"developer_msg"=>"Database error!!","ack_msg"=>"Failed! Unit Could Not Be Updated .");
					return $reply;
				}
			}	
		}	
	...
	public function DeleteUnit($detail)
	{
		...
		"isDelete"	=> "1"
			...
			if($uid!=0)
			{
				$reply=array("ack"=>1,"developer_msg"=>"Unit Record deleted!!","ack_msg"=>"Success! Delete Unit Successfully.");
				return $reply;
			}
			else
			{
