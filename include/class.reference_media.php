<?php
class ReferenceMedia extends Functions
{
	public $detail=array();
	public $db,$rights;
	public $ctable="reference_media";
	public $primary_column="id";
	public $unique_column="name";
	// Public Varibale
	public $id='';public $name='';public $slug='';public $adate=''; 
	public $valid_keys=array("id","name","slug");
	...
	function view($where="",$required_columns=array(),$orderby="",$limit="",$isSubCategoryRequired=false,$isProductRequired=false)
	{
        ...
        if($this->rights['view_flag']==1)
		{
			...
			if($result_r)
			{
				...
				$reply=array("ack"=>1,"developer_msg"=>"Reference Media detail fetched successfully!!","ack_msg"=>"Reference Media detail fetched successfully!!","result"=>$result,"total_count"=>$total_count,"show_count"=>$show_count);
				return $reply;
			}
			else
			{

				$reply=array("ack"=>0,"developer_msg"=>"Reference Media detail can not be fetch!","ack_msg"=>"Reference Media detail can not be fetch!");
				return $reply;
			}
		}
		else
		{
			...
			$reply=array("ack"=>0,"developer_msg"=>"Reference Media detail cannot be fetch!","ack_msg"=>"Reference Media detail cannot be fetch!");
			return $reply;
		}
		
	}
	function get_reference_media($where="",$required_columns=array(),$orderby="",$limit="")
	{
        ...
        if($this->rights['view_flag']==1)
		{
			...
			$show_count=$this->rp_getTotalRecord($this->ctable,$where,0,$this->getLimits($limit));
			// Get Actual Records
			$result_r=$this->db->rp_getData($this->ctable,"id,name",$where,$orderby,0,$this->getLimits($limit),0);
			if($result_r)
			{
				$data=array();
				while($d=mysqli_fetch_assoc($result_r))
				...
				if(!empty($detail))
				{
					...
					if($count<=0)
					{
						...
						if($inserted_id!=0)
						{
							...
							$reply=array("ack"=>1,"inserted_id"=>$inserted_id,"developer_msg"=>"Reference Media Detail inserted successfully!!","ack_msg"=>"Reference Media Detail inserted successfully!!");
							return $reply;
						}
						else
						{
							$reply=array("ack"=>0,"developer_msg"=>"Reference Media Detail can't be inserted!!","ack_msg"=>"Reference Media Detail can't be inserted!!");
							return $reply;
						}
					}	
					else
		...
		if($this->rights['update_flag']==1)
		{
			...
			if($validateKey['ack']==1)
			{
				...
				if(!empty($detail)&& $primary_key!="")
				{
					...
					if($count>=1)
					{
						$count=$this->duplicateCategory($this->unique_column,$unique_key,$primary_key);
						if($count<=0)
						{
							...
							if($isUpdated)
							{
    						    $reply=array("ack"=>1,"developer_msg"=>"Reference Media Detail updated successfully!!","ack_msg"=>"Reference Media Detail updated successfully!!");
								return $reply;
							}
							else
							{
								$reply=array("ack"=>0,"developer_msg"=>"Reference Media Detail cannot be updated!!","ack_msg"=>"Reference Media Detail cannot be updated!!");
								return $reply;
							}
						}
						else{
	...
	function delete($delete_array)
	{
		if($this->rights['delete_flag']==1)
		{
            ...
            if($count>=1)
            {
				...
				// $this->db->rp_delete("sub_reference_media","cid=".$delete_array['value']."",0);
				...
				$reply=array("ack"=>1,"developer_msg"=>"Reference Media deleted successfully!!","ack_msg"=>"Reference Media deleted successfully!!");
            	return $reply;
            }
            else
            {
		...
		{
			$this->db->rp_location("access_denied.php?msg=insert");
			$reply=array("ack"=>0,"developer_msg"=>"Reference Media cannot be fetch!","ack_msg"=>"Reference Media cannot be fetch!");
			return $reply;
		}		
	}
	...
	function active($active_array)
	{
		if($this->rights['update_flag']==1)
		{
				...
				if($count>=1)
				{
					...
					$reply=array("ack"=>1,"developer_msg"=>"Reference Media status updated successfully!!","ack_msg"=>"Reference Media status updated successfully!!");
					return $reply;
				}
				else
				{
	...
	{		
		$countReferenceMedia = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."' AND isDelete=0",0);
		return $countReferenceMedia;	
	}
	function duplicateCategory($key,$value,$primary_key)
	{		
		$countReferenceMedia = $this->db->rp_getTotalRecord($this->ctable,$key."='".$value."' AND isDelete=0 AND ".$this->primary_column."!=".$primary_key,0);
		return $countReferenceMedia;	
	}
