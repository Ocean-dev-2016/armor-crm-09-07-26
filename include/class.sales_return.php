...
class SalesReturn extends Functions
{
	public $db,$log,$product;
	public $ctable="sales_return"; 
	...
	public function AddInvocie($detail,$products)
	{
		...
		if($customer_d)
		{
			...
			$values = array(
				...
				isset($detail['original_invoice_id'])?$detail['original_invoice_id']:"",
					...
					foreach ($products as $p)
					{
						...
						if($product_detail)
						{
							...
							if($ctable_item_weight_detail)
							{
								...
								$item_id = $this->db->rp_insert("sales_return_item",$values,$rows,0);
							}
						}
					}
					$total_items=$this->db->rp_getTotalRecord("sales_return_item","invoice_id='".$order_id."' AND isDelete=0");
					if($total_items!=0)
					{
						$reply=array("ack"=>1,"developer_msg"=>"Data Added Successfully","ack_msg"=>"Data Added Successfully","order_id"=>$order_id);
						return $reply;
					}
					else
					{
						$reply=array("ack"=>0,"developer_msg"=>"Data Item Not inserted","ack_msg"=>"Data Item Not inserted");
						return $reply;
					}
				}
				else
	...
	{ 
		...
		$customer_r=mysqli_fetch_assoc($customer_d);
		$order_items_r=$this->db->rp_getData("sales_return_item","*","invoice_id='".$detail['order_id']."' AND isDelete=0","",0);
		if($order_items_r)
		{
			$total_qty=0;
			$sub_total=0;
...
// echo $grand_total;exit;
	...
	public function GetOrder($detail)
	{
		...
		$result['original_invoice_id']  		       = htmlentities($ctable_d['original_invoice_id']);
	...
	{		
		...
		$ctable_item = $this->db->rp_getData("sales_return_item","*",$where,"",0);
		if($ctable_item)
		{
	...
	public function UpdateInvoice($detail,$products)
	{
		...
		if($customer_d)
		{
			...
			$check_cart_exist=$this->db->rp_getTotalRecord($this->ctable,"id='".$detail['order_id']."' AND isDelete=0 ".$where,0);
			...
			if($check_cart_exist!=0)
			{
				if($detail['order_id']!=0)
				{
					...
					{
						// delete product
						$this->db->rp_delete("sales_return_item","invoice_id='".$order_id."'");
						// delete product
						...
						if($total_items!=0)
						{
							$reply=array("ack"=>1,"developer_msg"=>"Data Updated Successfully","ack_msg"=>"Data Updated Successfully","order_id"=>$order_id);
							return $reply;
						}
						else
						{
							$reply=array("ack"=>0,"developer_msg"=>"Data Item Not Updated","ack_msg"=>"Data Item Not Updated");
							return $reply;
						}
