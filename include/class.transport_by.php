...
class TransportBy extends Functions
{
	public $db;
	public $ctable="transport_by";
	...
	public function ADDTransportBy($detail) 
	{
        extract($detail);
	 ...
	 public function UpdateTransportBy($detail)
	  {
			extract($detail);
	...
	public function GetTransportBy($detail)
	{		
	...
	public function DeleteTransportBy($detail)
	{
		$rows 	= array(
		"isDelete"	=> "1"
		);
		...
		/*log entry*/
			$name = $this->db->rp_getValue("transport_by","name","id='".$_REQUEST['id']."'");
			$module_name = "Transport By";
			$flag = "Web";
