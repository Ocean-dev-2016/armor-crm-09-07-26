...
class Document extends Functions
{
	public $db;
	public $ctable="document_type";
	...
	public function InsertDesignation($detail) 
	{
		...
		if($r){
			$reply=array("ack"=>0,"developer_msg"=>"Already Exist Designation Name","ack_msg"=>"Duplication! Already Exist Document Type Name.");
			return $reply;
		}
		else
		{
			...
			if($uid!=0)
			{
				$reply=array("ack"=>1,"developer_msg"=>"Designation Added.","ack_msg"=>"Success! Document Type Insert Successfully.");
				return $reply;
			}
			else
			{
				$reply=array("ack"=>0,"developer_msg"=>"Database error!!","ack_msg"=>"Failed! Document Type Insert Failed.");
				return $reply;
			}
		}
	}
	 ...
	 public function UpdateDesignation($detail)
	  {
			...
			if($r){
				...
				if($uid!=0)
				{
					$reply=array("ack"=>1,"developer_msg"=>"Designation Update Successfull!!.","ack_msg"=>"Success! Document Update Successfully.");
					return $reply;
				}
				else
				{
					$reply=array("ack"=>0,"developer_msg"=>"Database error!!","ack_msg"=>"Failed! Document Update Failed.");
					return $reply;
				}
			}	
		}	
	...
	public function DeleteDesignation($detail)
	{
		...
		"isDelete"	=> "1"
			...
			$where	= "id='".$_REQUEST['id']."'";
			/*log entry*/
				$name = $this->db->rp_getValue("document_type","name","id='".$_REQUEST['id']."'");
			...
			if($uid!=0)
			{
				$reply=array("ack"=>1,"developer_msg"=>"deleted data.","ack_msg"=>"Success! Delete Document Type Successfully.");
				return $reply;
			}
			else
			{
				$reply=array("ack"=>0,"developer_msg"=>"Database error!!","ack_msg"=>"Failed! Delete Document Type Failed.");
				return $reply;
			}
	}
}
