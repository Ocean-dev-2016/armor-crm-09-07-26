...
class Designation extends Functions
{
	...
	public function InsertDesignation($detail) 
	{
		...
		if($r){
			$reply=array("ack"=>0,"developer_msg"=>"Already Exist Designation Name","ack_msg"=>"Duplication! Already Exist Purpose Name.");
			return $reply;
		}
		else
		{
			...
			if($uid!=0)
			{
				$reply=array("ack"=>1,"developer_msg"=>"Designation Added.","ack_msg"=>"Success! Purpose Insert Successfully.");
				return $reply;
			}
			else
			{
				$reply=array("ack"=>0,"developer_msg"=>"Database error!!","ack_msg"=>"Failed! Purpose Insert Failed.");
				return $reply;
			}
		}
	}
	 ...
	 public function UpdateDesignation($detail)
	  {
			...
			if($r){
				...
				if($uid!=0)
				{
					$reply=array("ack"=>1,"developer_msg"=>"Designation Update Successfull!!.","ack_msg"=>"Success! Purpose Update Successfully.");
					return $reply;
				}
				else
				{
					$reply=array("ack"=>0,"developer_msg"=>"Database error!!","ack_msg"=>"Failed! Purpose Update Failed.");
					return $reply;
				}
			}	
		}	
	...
	public function DeleteDesignation($detail)
	{
		...
		"isDelete"	=> "1"
			...
			$where	= "id='".$_REQUEST['id']."'";
			/*log entry*/
				$name = $this->db->rp_getValue("Purpose","name","id='".$_REQUEST['id']."'");
				$module_name = "Purpose";
				$flag = "Web";
			...
			if($uid!=0)
			{
				$reply=array("ack"=>1,"developer_msg"=>"deleted data.","ack_msg"=>"Success! Delete Purpose Successfully.");
				return $reply;
			}
			else
			{
