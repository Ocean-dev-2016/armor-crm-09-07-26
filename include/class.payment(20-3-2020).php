	...
	public function InsertPayment($detail)
	{
		...
		$adate	= date('Y-m-d H:i:s');
			...
			$values = array(
		 	...
		 	$uid = $this->db->rp_insert($this->ctable,$values,$rows,1);
			if($uid!=0)
			{
...
//--------#Update Payment Informstion------------------------------------------------//	 
	public function UpdatePayment($detail)
	  {
		extract($detail);
		$customer_id=$this->db->rp_getValue("payment","customer_id","id='".$id."'",0);
		$dispatch_id=$this->db->rp_getValue("payment","dispatch_id","id='".$id."'",0);
				$rows 	= array(
	...
	public function DeletePayment($detail)
	{
		...
		"isDelete"	=> "1"
			...
			if($uid!=0)
			{
				...
				$where	= "id='".$_REQUEST['id']."'";
				$updated=$this->db->rp_update("payment",$rows,$where,0);
